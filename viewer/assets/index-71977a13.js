...
fn preprocess(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if is_texd && atlas_params.atlas_width > 0u && atlas_params.probe_mode != 0u {
        // ---- Proberes: pass the pre-normalised affine straight through ----
        // Probe layout is row-major [A00, A01, A10, A11, t0, t1]; the exporter
        // already divided x-terms by width and y-terms by height, so there is
        // no arithmetic to do here at all. The off-diagonal pair rides in the
        // layer/_pad slots (safe: this atlas is single-layer, layer ≡ 0).
        let po = src_gauss * 6u;
...
struct Splat2DGS {
  ...
  uv_scale_x   : f32,
  uv_scale_y   : f32,
  // Baked path: \`layer\` is the atlas array slice, \`_pad\` unused.
  // Proberes (tex_params.probe_mode != 0): the shared 8192² texture is a
  // SINGLE layer, so \`layer\` is always 0 and both slots are free to carry the
  // probe affine's off-diagonal terms. Reinterpreted via bitcast — zero struct
  // growth, zero bandwidth change.
  //   layer -> uv_skew_x (A01/tex_w)   _pad -> uv_skew_y (A10/tex_h)
  layer        : u32,
  _pad         : u32,
};
...
struct VertexOutput {
  ...
  // Proberes off-diagonal terms (A01, A10), already divided by tex_res.
  // Zero under the baked path, where the affine is diagonal.
...
fn vs_main(
    ...
    // Under proberes the layer/_pad slots hold the affine's off-diagonal terms
    // (see Splat2DGS). Layer is always 0 there, so reading it as the skew is
    // safe; under the baked path probe_mode == 0 and uv_skew is never used.
...
fn shade(in: VertexOutput) -> ShadeOut {
    ...
    if tex_params.atlas_enabled != 0u {
        ...
        if tex_params.probe_mode != 0u {
            ...
            // mat2x2 takes COLUMNS. Probe layout is row-major
            // [A00, A01, A10, A11], so column 0 = (A00, A10) and
            // column 1 = (A01, A11). uv_scale = (A00, A11) diagonal,
            // uv_skew = (A01, A10) off-diagonal, all pre-divided by tex_res.
            let M = mat2x2<f32>(
                vec2<f32>(in.uv_scale.x, in.uv_skew.y),   // col 0: (A00, A10)
                vec2<f32>(in.uv_skew.x,  in.uv_scale.y),  // col 1: (A01, A11)
            );
            uv = in.uv_base + M * uv_eff;
            layer = 0;
        } else {
