...
async function main() {
    ...
    // --- bitymi bench fork: per-scene cameras (3DGS cameras.json format) ---
    if (viewerConfig.camerasUrl) {
        try {
            const cr = await fetch(viewerConfig.camerasUrl, { mode: "cors", credentials: "omit" });
            if (cr.ok) {
                cameras = await cr.json();
                camera = cameras[0];
                viewMatrix = getViewMatrix(camera);
                carousel = false;
                console.log("[bench] loaded", cameras.length, "cameras from", viewerConfig.camerasUrl);
            }
        } catch (err) { console.warn("[bench] cameras load failed", err); }
    }
    const modelBaseUrl =
    ...
    worker.onmessage = (e) => {
        if (e.data.progress) {
            ...
            if (window.__benchSortResolve) { const r = window.__benchSortResolve; window.__benchSortResolve = null; setTimeout(() => r(performance.now()), 0); }
            gl.bindBuffer(gl.ARRAY_BUFFER, indexBuffer);
    ...
    const frame = (now) => {
        if (window.__benchActive) { requestAnimationFrame(frame); return; }
        let inv = invert4(viewMatrix);
        let shiftKey =
    ...
    // ================= bitymi bench fork: nest-splatting FPS protocol =================
    // Per timed frame: teleport to the next camera of OUR camera list (cameras.json),
    // sort on the worker (timed, awaited), draw once at the scene's eval resolution,
    // sync GPU completion (gl.finish + 1-px readPixels; mobile GL drivers may return
    // from finish() early). 10 warmup + 200 timed frames, like benchmark_baked.py.
    const BENCH_RES = {
        bicycle:[1237,822], flowers:[1256,828], garden:[1297,840], stump:[1245,825], treehill:[1267,832],
        room:[1557,1038], counter:[1558,1038], kitchen:[1558,1039], bonsai:[1559,1039],
        train:[980,545], truck:[979,546], drjohnson:[1332,876], playroom:[1264,832],
    };
    window.__fluxBench = async function (opts = {}) {
        if (window.__benchActive) return null;
        const scene = viewerConfig.scene || "custom";
        const res = BENCH_RES[scene];
        const benchW = res ? res[0] : gl.canvas.width, benchH = res ? res[1] : gl.canvas.height;
        const nWarm = opts.warmup ?? 10, nBench = opts.frames ?? 200;
        const out = opts.onProgress || (() => {});
        if (vertexCount <= 0) { out("model still loading"); return null; }
        window.__benchActive = true;
        const savedW = gl.canvas.width, savedH = gl.canvas.height, savedView = viewMatrix.slice();
        try {
            gl.canvas.width = benchW; gl.canvas.height = benchH; gl.viewport(0, 0, benchW, benchH);
            gl.uniform2fv(u_viewport, new Float32Array([benchW, benchH]));
            const px = new Uint8Array(4);
            const sync = () => { gl.finish(); gl.readPixels(0, 0, 1, 1, gl.RGBA, gl.UNSIGNED_BYTE, px); };
            const sortMs = [], drawMs = [];
            const total = nWarm + nBench;
            for (let f = 0; f < total; f++) {
                const cam = cameras[f % cameras.length];
                const sx = benchW / cam.width, sy = benchH / cam.height;
                gl.uniform2fv(u_focal, new Float32Array([cam.fx * sx, cam.fy * sy]));
                const proj = getProjectionMatrix(cam.fx * sx, cam.fy * sy, benchW, benchH);
                gl.uniformMatrix4fv(u_projection, false, proj);
                const view = getViewMatrix(cam);
                const inv2 = invert4(view);
                gl.uniform3fv(u_camPos, [inv2[12], inv2[13], inv2[14]]);
                const viewProj = multiply4(proj, view);
                const t0 = performance.now();
                const sorted = new Promise((r) => { window.__benchSortResolve = r; });
                worker.postMessage({ view: viewProj });
                const t1 = await sorted;
                gl.uniformMatrix4fv(u_view, false, view);
                const t2 = performance.now();
                gl.clear(gl.COLOR_BUFFER_BIT);
                gl.activeTexture(gl.TEXTURE0); gl.bindTexture(gl.TEXTURE_2D, mainTexture);
                gl.activeTexture(gl.TEXTURE1); gl.bindTexture(gl.TEXTURE_2D, shTexture);
                gl.drawArraysInstanced(gl.TRIANGLE_FAN, 0, 4, vertexCount);
                sync();
                const t3 = performance.now();
                if (f >= nWarm) { sortMs.push(t1 - t0); drawMs.push(t3 - t2); }
                if (f % 10 === 0) { out(`${f < nWarm ? "warmup" : "bench"} ${f + 1}/${total}`); await new Promise((r) => setTimeout(r, 0)); }
            }
            const mean = (a) => a.reduce((x, y) => x + y, 0) / a.length;
            const med = (a) => { const b = [...a].sort((x, y) => x - y); return b[b.length >> 1]; };
            const dbg = gl.getExtension("WEBGL_debug_renderer_info");
            const gpu = dbg ? gl.getParameter(dbg.UNMASKED_RENDERER_WEBGL) : "unknown GPU";
            const r = {
                scene, gaussians: vertexCount, res: `${benchW}x${benchH}`, frames: nBench, warmup: nWarm,
                sort_ms: mean(sortMs), sort_ms_median: med(sortMs), draw_ms: mean(drawMs), draw_ms_median: med(drawMs),
                frame_ms: mean(sortMs) + mean(drawMs), gpu, ua: navigator.userAgent, cameras: cameras.length,
            };
            r.fps_new_view = 1000 / r.frame_ms;
            r.fps_draw_only = 1000 / r.draw_ms;
            r.fps_sort_bound = 1000 / r.sort_ms;
            console.log("[bench]", JSON.stringify(r));
            return r;
        } finally {
            gl.canvas.width = savedW; gl.canvas.height = savedH; gl.viewport(0, 0, savedW, savedH);
            gl.uniform2fv(u_viewport, new Float32Array([innerWidth, innerHeight]));
            gl.uniform2fv(u_focal, new Float32Array([camera.fx, camera.fy]));
            gl.uniformMatrix4fv(u_projection, false, projectionMatrix);
    ...
    window.__fluxInfo = () => ({ gaussians: vertexCount, cameras: cameras.length });
