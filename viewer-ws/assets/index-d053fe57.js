  ...
  @builtin(local_invocation_id)  lid : vec3<u32>
    ...
    if idx < arrayLength(&surfels) {
        ...
        if z_ndc > 0.0 && z_ndc < 1.0
            ...
            // scan barrier with \`alive = 0\`.
            //
            // Framebuffer coord convention (WebGPU): the rasterizer places a
            // fragment at fb.y = (1 − ndc.y)/2·H — +y_NDC is UP on screen,
            // fb.y = 0 is the TOP row. Since mesh_depth is written by the SAME
            // camera.proj·camera.view chain, its depth[fb.y][fb.x] follows the
            // same mapping. So to sample it we must use \`py_f = (1 − ndc.y)/2 · H\`
            // (NOT the naïve \`(ndc.y+1)/2·H\`, which would sample the vertically
            // mirrored row and only look right on top/bottom-symmetric geometry).
            // px keeps the standard mapping since NDC.x isn't flipped.
