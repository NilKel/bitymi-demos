...
`),Z="data:text/csv;charset=utf-8,"+encodeURIComponent(ue+j),$=document.createElement("a");$.href=Z,$.download=`${this.downloadOnceFileName}.csv`,document.body.appendChild($),$.click(),$.remove()}if(this.showPerfDialogNext){this.showPerfDialogNext=!1;try{alert(N)}catch{console.warn("Unable to show dialog; metrics printed to console.")}}this.resultBuffer.unmap()}_setupTimestampQueries(){this.querySet=this.device.createQuerySet({type:"timestamp",count:this.totalQueryCount});const l=this.totalQueryCount*8;this.resolveBuffer=this.device.createBuffer({size:l,usage:GPUBufferUsage.QUERY_RESOLVE|GPUBufferUsage.COPY_SRC}),this.resultBuffer=this.device.createBuffer({size:l,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ})}_setupBuffers(){this.render_settings_buffer=this.device.createBuffer({label:"render settings",size:rc,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const l=document.querySelector("canvas"),h=l?l.width:1,S=l?l.height:1;nc({width:h,height:S,sh_bias:this.pc.sh_bias,color_K:this.pc.K,feature_mode:this.pc.feature_mode}),fi(this.device,this.render_settings_buffer),this.splat_2d_buffer=this.device.createBuffer({label:"splats_2d (Splat2DGS)",size:Dn(this.pc.num_points*oc),usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.draw_indirect_buffer=this.device.createBuffer({label:"draw indirect",size:4*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC|GPUBufferUsage.INDIRECT}),this.device.queue.writeBuffer(this.draw_indirect_buffer,0,new Uint32Array([4,0,0,0])),this.sh_solvers_buffer=this.device.createBuffer({label:"sh_solvers",size:Dn(this.pc.num_points*ac),usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})}requestPerfDialog(){this.showPerfDialogNext=!0}requestDownloadMetrics(l){if(l&&l.trim().length>0){const h=l.trim().replace(/[^a-zA-Z0-9_\-]/g,"_");this.downloadOnceFileName=h.length>0?h:this.downloadOnceFileName}else{const h=new Date,S=`${h.getFullYear()}${String(h.getMonth()+1).padStart(2,"0")}${String(h.getDate()).padStart(2,"0")}_${String(h.getHours()).padStart(2,"0")}${String(h.getMinutes()).padStart(2,"0")}${String(h.getSeconds()).padStart(2,"0")}`;this.downloadOnceFileName=`fps_metrics_${S}`}this.downloadOnceNextRead=!0}requestReorder(){}async maybeReorderAfterSubmit(){}}function fc(o,l){return 2*Math.atan(l/(2*o))}function _c(o,l,h,S){const x=Math.tan(S/2),k=Math.tan(h/2),B=x*o,V=-B,D=k*o,A=-D,R=ct.create();return R[0]=2*o/(D-A),R[5]=-2*o/(B-V),R[2]=(D+A)/(D-A),R[6]=(B+V)/(B-V),R[14]=1,R[10]=l/(l-o),R[11]=-(l*o)/(l-o),ct.transpose(R,R),R}async function mc(o){Ct(`loading scene camera file... : ${o}`);const h=await(await fetch(o)).json();return Ct(`loaded cameras count: ${h.length}`),h.map(S=>{const x=L.clone(S.position),k=wt.create(...S.rotation.flat()),B=k[0],V=k[4],D=k[8],A=k[1],R=k[5],U=k[9],K=k[2],I=k[6],W=k[10];B*(R*W-U*I)-V*(A*W-U*K)+D*(A*I-R*K)<0&&(k[1]=-k[1],k[5]=-k[5],k[9]=-k[9]);const N=ct.fromMat3(k);return{position:x,rotation:N,img_name:S.img_name,id:S.id}})}const vc=4*2,bc=4*16,_i=4*bc+2*vc;function gc(o){return o.createBuffer({label:"camera uniform",size:_i,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})}const Jt=new Float32Array(_i/Float32Array.BYTES_PER_ELEMENT),Qn=class Qn{constructor(l,h){q(this,"uniform_buffer");q(this,"position",L.create());q(this,"rotation",ct.create());q(this,"fovY",45/180*Math.PI);q(this,"fovX");q(this,"focal",Hr.create());q(this,"viewport",Hr.create());q(this,"view_matrix",ct.identity());q(this,"view_inv_matrix",ct.identity());q(this,"proj_matrix",ct.identity());q(this,"proj_inv_matrix",ct.identity());q(this,"_negPos",L.create());q(this,"look",L.create(0,0,1));q(this,"up",L.create(0,1,0));q(this,"right",L.create(1,0,0));this.canvas=l,this.device=h,this.uniform_buffer=gc(h),this.on_update_canvas()}on_update_canvas(){const l=.5*this.canvas.height/Math.tan(this.fovY*.5);this.focal[0]=l,this.focal[1]=l,this.fovX=fc(l,this.canvas.width),this.viewport[0]=this.canvas.width,this.viewport[1]=this.canvas.height,this.proj_matrix=_c(.01,100,this.fovX,this.fovY),ct.inverse(this.proj_matrix,this.proj_inv_matrix),this.update_buffer()}update_buffer(){this._negPos[0]=-this.position[0],this._negPos[1]=-this.position[1],this._negPos[2]=-this.position[2],ct.copy(this.rotation,this.view_matrix),ct.translate(this.view_matrix,this._negPos,this.view_matrix),ct.inverse(this.view_matrix,this.view_inv_matrix),L.transformMat4Upper3x3(Qn.Z_AXIS,this.view_inv_matrix,this.look),L.normalize(this.look,this.look),L.cross(this.up,this.look,this.right),L.normalize(this.right,this.right);let l=0;Jt.set(this.view_matrix,l),l+=16,Jt.set(this.view_inv_matrix,l),l+=16,Jt.set(this.proj_matrix,l),l+=16,Jt.set(this.proj_inv_matrix,l),l+=16,Jt.set(this.viewport,l),l+=2,Jt.set(this.focal,l),l+=2,this.device.queue.writeBuffer(this.uniform_buffer,0,Jt)}set_preset(l){L.copy(l.position,this.position),ct.copy(l.rotation,this.rotation),this.update_buffer()}setFov(l){this.fovY=l,this.on_update_canvas()}getFov(){return this.fovY}};q(Qn,"Z_AXIS",L.create(0,0,1));let zs=Qn;const wc=L.create(1,0,0),xc=L.create(0,1,0);function yc(o,l){const h=o[0],S=o[4],x=o[8],k=o[1],B=o[5],V=o[9],D=o[2],A=o[6],R=o[10],U=h+B+R;let K,I,W,H;if(U>0){const N=.5/Math.sqrt(U+1);K=.25/N,I=(A-V)*N,W=(x-D)*N,H=(k-S)*N}else if(h>B&&h>R){const N=2*Math.sqrt(1+h-B-R);K=(A-V)/N,I=.25*N,W=(S+k)/N,H=(x+D)/N}else if(B>R){const N=2*Math.sqrt(1+B-h-R);K=(x-D)/N,I=(S+k)/N,W=.25*N,H=(V+A)/N}else{const N=2*Math.sqrt(1+R-h-B);K=(k-S)/N,I=(x+D)/N,W=(V+A)/N,H=.25*N}return l[0]=I,l[1]=W,l[2]=H,l[3]=K,l}class Pc{constructor(l){q(this,"element");q(this,"enabled",!0);q(this,"center",L.create(0,0,0));q(this,"up",L.create(0,1,0));q(this,"rotation",[0,0]);q(this,"shift",[0,0]);q(this,"scroll",0);q(this,"speed",.1);q(this,"sensitivity",.08);q(this,"leftPressed",!1);q(this,"rightPressed",!1);q(this,"leftDragPans",!1);q(this,"lastX",0);q(this,"lastY",0);q(this,"touches",new Map);q(this,"lastTouchCenter",null);q(this,"lastPinchDistance",null);q(this,"lastTwoFingerAngle",null);q(this,"lastTouchCount",0);q(this,"roll",0);q(this,"_dir",L.create());q(this,"_right",L.create());q(this,"_upCam",L.create());q(this,"_scratch",L.create());q(this,"_qY",pt.create());q(this,"_qX",pt.create());q(this,"_qRot",pt.create());q(this,"_qLocal",pt.create());q(this,"_qWorldToCam",pt.create());q(this,"_scratchMat3",wt.create());q(this,"bboxMin",null);q(this,"bboxMax",null);q(this,"anchor",L.create(0,0,0));q(this,"downCallback",l=>{var h,S,x,k;if(this.enabled){if(l.pointerType==="touch"){this.touches.set(l.pointerId,{x:l.pageX,y:l.pageY}),this.handleTouchGestures(),(S=(h=l.target)==null?void 0:h.setPointerCapture)==null||S.call(h,l.pointerId),l.preventDefault();return}l.isPrimary&&(l.button===0?(this.leftPressed=!0,this.leftDragPans=l.shiftKey):l.button===2?this.rightPressed=!0:this.rightPressed=!0,this.lastX=l.pageX,this.lastY=l.pageY,(k=(x=l.target)==null?void 0:x.setPointerCapture)==null||k.call(x,l.pointerId),l.preventDefault())}});q(this,"moveCallback",l=>{if(!this.enabled)return;if(l.pointerType==="touch"){if(!this.touches.has(l.pointerId))return;this.touches.set(l.pointerId,{x:l.pageX,y:l.pageY}),this.handleTouchGestures(),l.preventDefault();return}if(!l.isPrimary||!this.leftPressed&&!this.rightPressed)return;l.preventDefault();const h=l.pageX-this.lastX,S=l.pageY-this.lastY;this.lastX=l.pageX,this.lastY=l.pageY,this.leftPressed&&!this.leftDragPans?(this.rotation[0]+=h,this.rotation[1]-=S):(this.rightPressed||this.leftPressed&&this.leftDragPans)&&(this.shift[1]-=h,this.shift[0]+=S)});q(this,"upCallback",l=>{var h,S,x,k;if(l.pointerType==="touch"){this.touches.delete(l.pointerId),this.handleTouchGestures(),(S=(h=l.target)==null?void 0:h.releasePointerCapture)==null||S.call(h,l.pointerId),l.preventDefault();return}l.button===0?this.leftPressed=!1:l.button===2?this.rightPressed=!1:this.rightPressed=!1,(k=(x=l.target)==null?void 0:x.releasePointerCapture)==null||k.call(x,l.pointerId),l.preventDefault()});q(this,"wheelCallback",l=>{if(!this.enabled||(l.preventDefault(),this.rightPressed))return;let h=l.deltaY;l.deltaMode===1?h*=16:l.deltaMode===2&&(h*=100),this.scroll+=h*.01});this.camera=l,this.registerElement(l.canvas)}registerElement(l){this.element&&this.element!==l&&(this.element.removeEventListener("pointerdown",this.downCallback),this.element.removeEventListener("pointermove",this.moveCallback),this.element.removeEventListener("pointerup",this.upCallback),this.element.removeEventListener("wheel",this.wheelCallback)),this.element=l,this.element.addEventListener("pointerdown",this.downCallback),this.element.addEventListener("pointermove",this.moveCallback),this.element.addEventListener("pointerup",this.upCallback),this.element.addEventListener("wheel",this.wheelCallback,{passive:!1}),this.element.addEventListener("contextmenu",h=>h.preventDefault())}setCenter(l){L.copy(l,this.center),L.copy(l,this.anchor)}setOrbitPivot(l){L.set(l[0],l[1],l[2],this.center),this._reorientCameraToCenter()}setOrbitDepth(l){if(!isFinite(l)||l<.001)return;const h=this.camera.rotation;L.set(h[2],h[6],h[10],this._dir),L.normalize(this._dir,this._dir),L.scale(this._dir,l,this._dir),L.add(this.camera.position,this._dir,this.center)}_reorientCameraToCenter(){const l=this.camera;if(L.subtract(this.center,l.position,this._scratch),L.length(this._scratch)<1e-6)return;L.normalize(this._scratch,this._scratch),L.cross(this.up,this._scratch,this._right),L.length(this._right)<1e-6&&L.set(1,0,0,this._right),L.normalize(this._right,this._right),L.cross(this._scratch,this._right,this._upCam),L.normalize(this._upCam,this._upCam);const h=l.rotation;h[0]=this._right[0],h[1]=this._upCam[0],h[2]=this._scratch[0],h[3]=0,h[4]=this._right[1],h[5]=this._upCam[1],h[6]=this._scratch[1],h[7]=0,h[8]=this._right[2],h[9]=this._upCam[2],h[10]=this._scratch[2],h[11]=0,h[12]=0,h[13]=0,h[14]=0,h[15]=1,l.update_buffer()}setBbox(l,h){this.bboxMin=L.create(l[0],l[1],l[2]),this.bboxMax=L.create(h[0],h[1],h[2]);const S=(l[0]+h[0])*.5,x=(l[1]+h[1])*.5,k=(l[2]+h[2])*.5;L.set(S,x,k,this.center),L.set(S,x,k,this.anchor)}resetToCamera(){const l=this.camera.rotation;L.set(l[2],l[6],l[10],this._dir),L.normalize(this._dir,this._dir);let h=null;if(this.bboxMin&&this.bboxMax){let S=-1/0,x=1/0,k=!1;for(let B=0;B<3;B++){const V=this._dir[B],D=this.bboxMin[B]-this.camera.position[B],A=this.bboxMax[B]-this.camera.position[B];if(Math.abs(V)>1e-8){const R=D/V,U=A/V;S=Math.max(S,Math.min(R,U)),x=Math.min(x,Math.max(R,U))}else if(D>0||A<0){k=!0;break}}!k&&S<=x&&x>0&&(h=(Math.max(S,0)+x)*.5)}if(h===null||!isFinite(h)||h<.001){L.subtract(this.anchor,this.camera.position,this._scratch);const S=L.dot(this._scratch,this._dir);h=S>.001?S:L.length(this._scratch)}h=Math.max(.1,h),L.scale(this._dir,h,this._dir),L.add(this.camera.position,this._dir,this.center)}handleTouchGestures(){const l=this.touches.size;if(l!==this.lastTouchCount&&(this.lastTouchCenter=null,this.lastPinchDistance=null,this.lastTwoFingerAngle=null),this.lastTouchCount=l,l===1){const h=this.touches.values().next().value;if(this.lastTouchCenter){const S=h.x-this.lastTouchCenter[0],x=h.y-this.lastTouchCenter[1];this.rotation[0]+=S*.3,this.rotation[1]-=x*.3}this.lastTouchCenter=[h.x,h.y]}else if(l===2){const h=Array.from(this.touches.values()),S=(h[0].x+h[1].x)*.5,x=(h[0].y+h[1].y)*.5,k=h[1].x-h[0].x,B=h[1].y-h[0].y,V=Math.hypot(k,B),D=Math.atan2(B,k);if(this.lastTouchCenter!==null&&this.lastPinchDistance!==null&&this.lastTwoFingerAngle!==null){const A=S-this.lastTouchCenter[0],R=x-this.lastTouchCenter[1],U=Math.hypot(A,R),K=Math.abs(V-this.lastPinchDistance);let I=D-this.lastTwoFingerAngle;I>Math.PI&&(I-=2*Math.PI),I<-Math.PI&&(I+=2*Math.PI),U>.5&&(this.shift[1]-=A,this.shift[0]+=R),K>1&&this.lastPinchDistance>.001&&(this.scroll+=-Math.log(V/this.lastPinchDistance)*10),Math.abs(I)>.0087&&(this.roll+=-I)}this.lastTouchCenter=[S,x],this.lastPinchDistance=V,this.lastTwoFingerAngle=D}}update(l){if(!this.enabled||Math.abs(this.rotation[0])<1e-4&&Math.abs(this.rotation[1])<1e-4&&Math.abs(this.shift[0])<1e-4&&Math.abs(this.shift[1])<1e-4&&Math.abs(this.scroll)<1e-4&&Math.abs(this.roll)<1e-4)return;const h=this.camera;{const H=h.rotation;this.up[0]=H[1],this.up[1]=H[5],this.up[2]=H[9],L.length(this.up)>1e-6?L.normalize(this.up,this.up):L.set(0,1,0,this.up)}let S=!1;if(Math.abs(this.roll)>1e-4){const H=h.rotation;L.set(H[2],H[6],H[10],this._scratch),L.normalize(this._scratch,this._scratch),pt.fromAxisAngle(this._scratch,this.roll,this._qRot),L.transformQuat(this.up,this._qRot,this.up),L.normalize(this.up,this.up),this.roll=0,S=!0}L.subtract(h.position,this.center,this._dir);let x=L.length(this._dir);x<1e-6&&(x=1e-6);const k=Math.exp(Math.log(x)+this.scroll*l*10*this.speed);L.scale(this._dir,k/x,this._dir),x=k;const B=h.rotation;this._right[0]=B[0],this._right[1]=B[4],this._right[2]=B[8],L.normalize(this._right,this._right),L.length(this._right)<1e-6&&L.set(1,0,0,this._right);const V=L.create(B[1],B[5],B[9]);L.normalize(V,V),L.length(V)<1e-6&&L.set(0,1,0,V);const D=l*this.speed*.1*x,A=this.shift[1]*D,R=-this.shift[0]*D;L.scale(this._right,A,this._scratch),L.add(this.center,this._scratch,this.center),L.add(h.position,this._scratch,h.position),L.scale(V,R,this._scratch),L.add(this.center,this._scratch,this.center),L.add(h.position,this._scratch,h.position);const U=this.rotation[0]*l*this.sensitivity,K=this.rotation[1]*l*this.sensitivity;if(Math.abs(U)>1e-5||Math.abs(K)>1e-5||S){const H=h.rotation;yc(H,this._qWorldToCam),pt.fromAxisAngle(wc,-K,this._qX),pt.fromAxisAngle(xc,-U,this._qY),pt.multiply(this._qX,this._qY,this._qLocal),pt.normalize(this._qLocal,this._qLocal),pt.multiply(this._qLocal,this._qWorldToCam,this._qWorldToCam),pt.normalize(this._qWorldToCam,this._qWorldToCam),wt.fromQuat(this._qWorldToCam,this._scratchMat3),ct.fromMat3(this._scratchMat3,h.rotation);const N=h.rotation,ue=N[2],j=N[6],Z=N[10];h.position[0]=this.center[0]-ue*x,h.position[1]=this.center[1]-j*x,h.position[2]=this.center[2]-Z*x,this.up[0]=N[1],this.up[1]=N[5],this.up[2]=N[9],L.normalize(this.up,this.up)}else L.add(this.center,this._dir,h.position);h.update_buffer();const W=Math.pow(.8,l*60);this.rotation[0]*=W,Math.abs(this.rotation[0])<1e-4&&(this.rotation[0]=0),this.rotation[1]*=W,Math.abs(this.rotation[1])<1e-4&&(this.rotation[1]=0),this.shift[0]*=W,Math.abs(this.shift[0])<1e-4&&(this.shift[0]=0),this.shift[1]*=W,Math.abs(this.shift[1])<1e-4&&(this.shift[1]=0),this.scroll*=W,Math.abs(this.scroll)<1e-4&&(this.scroll=0)}}function mi(o){const l=L.create();for(const h of o)L.add(l,h,l);return L.scale(l,1/Math.max(o.length,1),l)}function vi(o,l){const h=wt.create();wt.inverse(o,h);const S=L.create();return S[0]=h[0]*l[0]+h[4]*l[1]+h[8]*l[2],S[1]=h[1]*l[0]+h[5]*l[1]+h[9]*l[2],S[2]=h[2]*l[0]+h[6]*l[1]+h[10]*l[2],S}function Sc(o){const l=o.slice(),h=[1,0,0,0,1,0,0,0,1],S=(D,A)=>l[D*3+A],x=(D,A,R)=>{l[D*3+A]=R},k=(D,A)=>h[D*3+A],B=(D,A,R)=>{h[D*3+A]=R};for(let D=0;D<30;D++){let A=0,R=1,U=Math.abs(S(0,1));if(Math.abs(S(0,2))>U&&(A=0,R=2,U=Math.abs(S(0,2))),Math.abs(S(1,2))>U&&(A=1,R=2,U=Math.abs(S(1,2))),U<1e-12)break;const K=S(A,A),I=S(R,R),W=S(A,R);let H;Math.abs(K-I)<1e-30?H=Math.PI/4*Math.sign(W):H=.5*Math.atan2(2*W,K-I);const N=Math.cos(H),ue=Math.sin(H);for(let j=0;j<3;j++){const Z=S(j,A),$=S(j,R);x(j,A,N*Z+ue*$),x(j,R,-ue*Z+N*$)}for(let j=0;j<3;j++){const Z=S(A,j),$=S(R,j);x(A,j,N*Z+ue*$),x(R,j,-ue*Z+N*$)}for(let j=0;j<3;j++){const Z=k(j,A),$=k(j,R);B(j,A,N*Z+ue*$),B(j,R,-ue*Z+N*$)}}const V=[];for(let D=0;D<3;D++)V.push({val:S(D,D),vec:L.create(k(0,D),k(1,D),k(2,D))});return V.sort((D,A)=>A.val-D.val),{vals:[V[0].val,V[1].val,V[2].val],vecs:[V[0].vec,V[1].vec,V[2].vec]}}function Cc(o,l){const h=mi(o);let S=0,x=0,k=0,B=0,V=0,D=0;for(const W of o){const H=W[0]-h[0],N=W[1]-h[1],ue=W[2]-h[2];S+=H*H,x+=H*N,k+=H*ue,B+=N*N,V+=N*ue,D+=ue*ue}const A=[S,x,k,x,B,V,k,V,D],{vecs:R}=Sc(A);let U=R[0],K=R[1],I=R[2];return L.dot(I,l)<0&&(L.scale(I,-1,I),L.scale(K,-1,K)),{centroid:h,normal:I,u:U,v:K}}function Ec(o){let l=0,h=0,S=0,x=0,k=0,B=0,V=0,D=0,A=0;for(const[N,ue]of o){const j=-2*N,Z=-2*ue,$=1,ce=-(N*N+ue*ue);l+=j*j,h+=j*Z,S+=j*$,x+=Z*Z,k+=Z*$,B+=$*$,V+=j*ce,D+=Z*ce,A+=$*ce}const R=wt.create(l,h,S,h,x,k,S,k,B),U=vi(R,L.create(V,D,A)),K=U[0],I=U[1],W=U[2],H=K*K+I*I-W;return{center:[K,I],radius:Math.sqrt(Math.max(H,1e-12))}}function kc(o,l){let h=0,S=0,x=0,k=0,B=0,V=0,D=0,A=0,R=0;for(let K=0;K<o.length;K++){const I=o[K],W=L.normalize(l[K],L.create()),H=1-W[0]*W[0],N=-W[0]*W[1],ue=-W[0]*W[2],j=1-W[1]*W[1],Z=-W[1]*W[2],$=1-W[2]*W[2];h+=H,S+=N,x+=ue,k+=j,B+=Z,V+=$,D+=H*I[0]+N*I[1]+ue*I[2],A+=N*I[0]+j*I[1]+Z*I[2],R+=ue*I[0]+Z*I[1]+$*I[2]}const U=wt.create(h,S,x,S,k,B,x,B,V);return vi(U,L.create(D,A,R))}function bi(o,l={}){if(o.length===0)return null;const h=l.tiltDownDeg??8,S=l.radiusScale??1,x=l.alignFirst??!0,k=(l.direction??"ccw")==="ccw"?1:-1,B=o.map(oe=>L.clone(oe.position)),V=o.map(oe=>{const de=oe.rotation;return L.create(de[8],de[9],de[10])}),D=o.map(oe=>{const de=oe.rotation;return L.create(de[4],de[5],de[6])}),A=mi(D),R=L.normalize(L.scale(A,-1,L.create())),{centroid:U,normal:K,u:I,v:W}=Cc(B,R),H=B.map(oe=>{const de=L.sub(oe,U,L.create());return[L.dot(de,I),L.dot(de,W)]}),{center:N,radius:ue}=Ec(H),j=ue*S,Z=L.add(U,L.add(L.scale(I,N[0],L.create()),L.scale(W,N[1],L.create()),L.create()),L.create()),$=kc(B,V),ce=j*Math.tan(h*Math.PI/180),se=L.sub($,L.scale(K,ce,L.create()),L.create());let pe=0;if(x){const oe=L.sub(B[0],Z,L.create());pe=Math.atan2(L.dot(oe,W),L.dot(oe,I))/(2*Math.PI)%1,pe<0&&(pe+=1)}return console.log(`[orbit] fit ${o.length} train cams: radius=${j.toFixed(2)}, tilt=${h}°, normal=[${K[0].toFixed(2)}, ${K[1].toFixed(2)}, ${K[2].toFixed(2)}], startPhase=${pe.toFixed(3)}`),{center:Z,radius:j,normal:K,u:I,v:W,lookAt:se,startPhase:pe,direction:k}}function gi(o,l){const h=(o.startPhase+l*o.direction)*2*Math.PI,S=Math.cos(h),x=Math.sin(h),k=L.add(o.center,L.add(L.scale(o.u,o.radius*S,L.create()),L.scale(o.v,o.radius*x,L.create()),L.create()),L.create()),B=L.normalize(L.sub(o.lookAt,k,L.create())),V=L.cross(B,o.normal,L.create());L.length(V)<1e-6&&L.copy(o.u,V),L.normalize(V,V);const D=L.cross(B,V,L.create());L.normalize(D,D);const A=ct.create();return A[0]=V[0],A[1]=D[0],A[2]=B[0],A[3]=0,A[4]=V[1],A[5]=D[1],A[6]=B[1],A[7]=0,A[8]=V[2],A[9]=D[2],A[10]=B[2],A[11]=0,A[12]=0,A[13]=0,A[14]=0,A[15]=1,{position:k,rotation:A,img_name:`orbit_${(l*1e3).toFixed(0)}`,id:0}}function Mc(o,l={}){const h=bi(o,l);if(!h)return[];const S=l.numViews??120;return Array.from({length:S},(x,k)=>({...gi(h,k/S),img_name:`circle_${k.toString().padStart(4,"0")}`,id:k}))}function Bc(o){const l=new Uint8Array(o),h=Math.min(l.byteLength,65536),S=new TextDecoder("ascii").decode(l.subarray(0,h)),x=S.indexOf("end_header");if(x<0)throw new Error("mesh PLY: 'end_header' not found in first 64KB");const k=x+10+1,B=S.slice(0,x).split(/\r?\n/).map(ie=>ie.trim()).filter(Boolean);if(B[0]!=="ply")throw new Error("mesh PLY: missing 'ply' magic");const V=B.find(ie=>ie.startsWith("format"));if(!V||!V.includes("binary_little_endian"))throw new Error(`mesh PLY: only binary_little_endian supported (got: ${V??"<none>"})`);const D={char:1,uchar:1,int8:1,uint8:1,short:2,ushort:2,int16:2,uint16:2,int:4,uint:4,int32:4,uint32:4,float:4,float32:4,double:8,float64:8},A=[];let R=null;for(const ie of B)if(ie.startsWith("element ")){const[,me,Le]=ie.split(/\s+/);R={name:me,count:parseInt(Le,10),props:[],bytesPerRecord:0,hasList:!1},A.push(R)}else if(ie.startsWith("property ")&&R){const me=ie.split(/\s+/);if(me[1]==="list"){const Le=me[2],re=me[3],_e=me[4];R.props.push({name:_e,type:re,isList:!0,listCountType:Le,listItemType:re}),R.hasList=!0}else{const Le=me[1],re=me[2];if(R.props.push({name:re,type:Le,isList:!1}),D[Le]===void 0)throw new Error(`mesh PLY: unsupported prop type '${Le}'`);R.bytesPerRecord+=D[Le]}}const U=A.find(ie=>ie.name==="vertex"),K=A.find(ie=>ie.name==="face"||ie.name==="faces");if(!U)throw new Error("mesh PLY: missing 'element vertex'");if(!K)throw new Error("mesh PLY: missing 'element face' (mesh has no triangles)");if(U.hasList)throw new Error("mesh PLY: unexpected list property on vertex element");const I=U.props.find(ie=>ie.name==="x"),W=U.props.find(ie=>ie.name==="y"),H=U.props.find(ie=>ie.name==="z");if(!I||!W||!H)throw new Error("mesh PLY: vertex must have x/y/z");let N=0;const ue=new Map;for(const ie of U.props)ue.set(ie.name,{off:N,type:ie.type}),N+=D[ie.type];const j=ue.get("x"),Z=ue.get("y"),$=ue.get("z");if(j.type!==Z.type||j.type!==$.type)throw new Error(`mesh PLY: x/y/z must share a type (got ${j.type}/${Z.type}/${$.type})`);const ce=j.type,se=ce==="double"||ce==="float64";if(!se&&ce!=="float"&&ce!=="float32")throw new Error(`mesh PLY: x/y/z must be float or double (got '${ce}')`);const pe=new Float32Array(U.count*3),oe=new DataView(o),de=k,Ee=U.bytesPerRecord;for(let ie=0;ie<U.count;ie++){const me=de+ie*Ee;se?(pe[ie*3+0]=oe.getFloat64(me+j.off,!0),pe[ie*3+1]=oe.getFloat64(me+Z.off,!0),pe[ie*3+2]=oe.getFloat64(me+$.off,!0)):(pe[ie*3+0]=oe.getFloat32(me+j.off,!0),pe[ie*3+1]=oe.getFloat32(me+Z.off,!0),pe[ie*3+2]=oe.getFloat32(me+$.off,!0))}const Ue=de+U.count*Ee,Ie=K.props.find(ie=>ie.isList);if(!Ie)throw new Error("mesh PLY: face element has no list property");const Re=D[Ie.listCountType],Be=D[Ie.listItemType];if(Re!==1)throw new Error(`mesh PLY: unsupported face list count size ${Re}`);let we=Ue,Ce=0;for(let ie=0;ie<K.count;ie++){const me=oe.getUint8(we);me===3&&Ce++,we+=1+me*Be}const Q=new Uint32Array(Ce*3);we=Ue;let ee=0;for(let ie=0;ie<K.count;ie++){const me=oe.getUint8(we);if(we+=1,me===3)for(let Le=0;Le<3;Le++){const re=we+Le*Be;let _e;if(Be===4)_e=oe.getUint32(re,!0);else if(Be===2)_e=oe.getUint16(re,!0);else if(Be===1)_e=oe.getUint8(re);else throw new Error(`mesh PLY: unsupported face index size ${Be}`);Q[ee++]=_e}we+=me*Be}return{positions:pe,indices:Q}}function Dc(o){const l=new DataView(o);if(o.byteLength<8)throw new Error("mesh .bin: too small (need at least 8-byte header)");const h=l.getUint32(0,!0),S=l.getUint32(4,!0),x=h*12,k=S*4;if(o.byteLength<8+x+k)throw new Error(`mesh .bin: truncated (expected ${8+x+k} B, got ${o.byteLength} B)`);const B=new Float32Array(o.slice(8,8+x)),V=new Uint32Array(o.slice(8+x,8+x+k));return{positions:B,indices:V}}function wi(o,l){return(l??"").toLowerCase().endsWith(".ply")?Bc(o):Dc(o)}function Tc(o,l){const h=o.length/3,S=new Float32Array(h*3);for(let x=0;x<l.length;x+=3){const k=l[x]*3,B=l[x+1]*3,V=l[x+2]*3,D=o[k],A=o[k+1],R=o[k+2],U=o[B],K=o[B+1],I=o[B+2],W=o[V],H=o[V+1],N=o[V+2],ue=U-D,j=K-A,Z=I-R,$=W-D,ce=H-A,se=N-R,pe=j*se-Z*ce,oe=Z*$-ue*se,de=ue*ce-j*$;S[k]+=pe,S[k+1]+=oe,S[k+2]+=de,S[B]+=pe,S[B+1]+=oe,S[B+2]+=de,S[V]+=pe,S[V+1]+=oe,S[V+2]+=de}for(let x=0;x<h;x++){const k=x*3,B=S[k],V=S[k+1],D=S[k+2],A=Math.sqrt(B*B+V*V+D*D);if(A>1e-12){const R=1/A;S[k]=B*R,S[k+1]=V*R,S[k+2]=D*R}}return S}function xi(o,l){const h=o.createBuffer({label:"mesh-cull vertex buffer",size:l.positions.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});o.queue.writeBuffer(h,0,l.positions);const S=Tc(l.positions,l.indices),x=o.createBuffer({label:"mesh-cull normal buffer",size:S.byteLength,usage:GPUBufferUsage.VERTEX|GPUBufferUsage.COPY_DST});o.queue.writeBuffer(x,0,S);const k=o.createBuffer({label:"mesh-cull index buffer",size:l.indices.byteLength,usage:GPUBufferUsage.INDEX|GPUBufferUsage.COPY_DST});return o.queue.writeBuffer(k,0,l.indices),{vertexBuffer:h,normalBuffer:x,indexBuffer:k,numVerts:l.positions.length/3,numIndices:l.indices.length}}async function Ac(o,l){try{const h=await fetch(l);if(!h.ok)return console.warn(`[mesh-cull] fetch failed for ${l}: ${h.status}`),null;const S=await h.arrayBuffer(),x=wi(S,l);return console.log(`[mesh-cull] loaded ${l}: ${x.positions.length/3} verts, ${x.indices.length/3} tris`),xi(o,x)}catch(h){return console.warn(`[mesh-cull] load error for ${l}:`,h),null}}function zc(o,l,h){try{const S=wi(l,h);return console.log(`[mesh-cull] loaded from bundle: ${S.positions.length/3} verts, ${S.indices.length/3} tris`),xi(o,S)}catch(S){return console.warn("[mesh-cull] parse error:",S),null}}const yi="BITYMI01",Lc=0,Ic=1,Gc=2,Uc=3,Rc=4,Vc=5,Oc=6;function Yn(o){const l=(o&32768)>>15,h=(o&31744)>>10,S=o&1023;return h===0?(l?-1:1)*Math.pow(2,-14)*(S/1024):h===31?S?NaN:l?-1/0:1/0:(l?-1:1)*Math.pow(2,h-15)*(1+S/1024)}function ri(o,l,h,S,x,k){const B=(o-h.width*.5)/S.focal[0],V=-((l-h.height*.5)/S.focal[1]),D=S.rotation,A=D[0],R=D[4],U=D[8],K=D[1],I=D[5],W=D[9],H=D[2],N=D[6],ue=D[10];let j=B*A+V*K+H,Z=B*R+V*I+N,$=B*U+V*W+ue;const ce=Math.hypot(j,Z,$)||1;j/=ce,Z/=ce,$/=ce;const se=S.position[0],pe=S.position[1],oe=S.position[2],de=k*.005,Ee=de*de,Ue=new Uint32Array(x.buffer,x.byteOffset,x.length),Ie=x.length/8;let Re=1/0,Be=0,we=0,Ce=0,Q=-1;for(let ee=0;ee<Ie;ee++){const ie=ee*8,me=x[ie+0]-se,Le=x[ie+1]-pe,re=x[ie+2]-oe,_e=me*j+Le*Z+re*$;if(_e<=0)continue;const ge=me-_e*j,De=Le-_e*Z,le=re-_e*$;if(ge*ge+De*De+le*le>=Ee||!(Ue[ie+7]>>>16&1))continue;const w=Ue[ie+5],i=Ue[ie+6],f=Yn(w&65535),d=Yn(w>>>16&65535),b=Yn(i&65535),g=Yn(i>>>16&65535),P=Math.hypot(f,d,b,g)||1,E=f/P,r=d/P,m=b/P,u=g/P,p=2*(r*u+E*m),a=2*(m*u-E*r),_=1-2*(r*r+m*m),y=j*p+Z*a+$*_;let v;Math.abs(y)>1e-6?(v=(me*p+Le*a+re*_)/y,(!isFinite(v)||v<=0)&&(v=_e)):v=_e,v<Re&&(Re=v,Be=se+v*j,we=pe+v*Z,Ce=oe+v*$,Q=ee)}return Q<0?null:[Be,we,Ce]}function Pi(o){const l=new Uint8Array(o),h=new TextDecoder().decode(l.subarray(0,8));if(h!==yi)throw new Error(`Not a BITYMI bundle (bad magic '${h}')`);const S=new DataView(o),x=S.getUint32(8,!0),k=12,B=20;let V=null,D=null,A=null,R=null;for(let U=0;U<x;U++){const K=k+U*B,I=S.getUint32(K+0,!0),W=Number(S.getBigUint64(K+4,!0)),H=Number(S.getBigUint64(K+12,!0)),N=l.slice(W,W+H).buffer;I===Lc||I===Ic||I===Vc?V=N:I===Gc?D=N:I===Uc||I===Rc?A=N:I===Oc&&(R=N)}if(V===null)throw new Error("BITYMI bundle has no point cloud chunk");return{pcBuffer:V,camerasBuffer:D,atlasBuffer:A,meshBuffer:R}}async function Fc(o,l){var B;const h=await fetch(o);if(!h.ok)throw new Error(`fetch failed: ${h.status} ${h.statusText}`);const S=(()=>{const V=h.headers.get("content-length");return V&&parseInt(V,10)||void 0})(),x=(B=h.body)==null?void 0:B.getReader();let k;if(!x)k=await h.arrayBuffer(),l&&l(k.byteLength,S,0);else{const V=[];let D=0,A=performance.now(),R=0;for(;;){const{done:I,value:W}=await x.read();if(I)break;V.push(W),D+=W.byteLength;const H=performance.now();if(H-A>=150&&l){const N=(D-R)/((H-A)/1e3);l(D,S,N),A=H,R=D}}const U=new Uint8Array(D);let K=0;for(const I of V)U.set(I,K),K+=I.byteLength;k=U.buffer,l&&l(D,S,0)}return k.byteLength>=8&&new TextDecoder().decode(new Uint8Array(k,0,8))===yi?{bundle:Pi(k),rawPly:null}:{bundle:null,rawPly:k}}function Nc(o){return new Promise(l=>{const h=document.createElement("input");h.type="file",h.accept=o,h.style.display="none",h.onchange=()=>{var S;return l(((S=h.files)==null?void 0:S[0])??null)},document.body.appendChild(h),h.click(),setTimeout(()=>document.body.removeChild(h),1e3)})}function $c(o,l,h){const S=document.getElementById("ui-panel-container"),x=document.getElementById("load-button"),k=document.getElementById("quick-links");x&&(x.onclick=async()=>{const R=await Nc(".ply,.bitymi");if(R)if(S&&(S.style.display="none"),R.name.toLowerCase().endsWith(".bitymi")){const U=await R.arrayBuffer(),{pcBuffer:K}=Pi(U),I=new File([K],R.name.replace(/\.bitymi$/i,".ply"),{type:"application/octet-stream"}),W=await Ms(I,o);l(W)}else{const U=await Ms(R,o);l(U)}}),k&&(k.innerHTML="");const B=new URLSearchParams(window.location.search),V=B.get("bundle")??B.get("model_url"),D=B.get("camera_url"),A=B.get("mesh_url");V&&(S&&(S.style.display="none"),h(V,D,A))}async function qc(o,l,h,S){const x=new zs(o,h),k=new Pc(x);let B=!1;o.addEventListener("pointerdown",()=>{B=!0}),window.addEventListener("pointerup",()=>{B=!1});const V="rgba8unorm";l.configure({device:h,format:V,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.STORAGE_BINDING});let D=null;const A=()=>{x.on_update_canvas(),D!==null&&Kn(o.width,o.height,h,D.render_settings_buffer)};new ResizeObserver(()=>{const le=Math.max(.25,re.render_scale),M=Math.max(1,Math.ceil(le*o.clientWidth)),G=Math.max(1,Math.ceil(le*o.clientHeight));o.width===M&&o.height===G||(o.width=M,o.height=G,A())}).observe(o);let U=0,K=0;const I=()=>{(o.width!==U||o.height!==K)&&(U=o.width,K=o.height,A())},W=new URLSearchParams(window.location.search);let N=W.get("animation")==="1";k.enabled=!N;const ue=W.get("camera_url"),j=W.get("mesh_cull"),Z=j==="1"||j==="true",$=W.get("mesh_normal_margin")??W.get("mesh_margin"),ce=$!==null?Number($):NaN,se=Number.isFinite(ce)?ce:0,pe=W.get("mesh_debug"),oe=pe==="1"||pe==="true",de=W.get("bfc"),Ee=de==="1"||de==="true",Ue=W.get("bfc_cos"),Ie=Ue!==null?Number(Ue):NaN,Re=Number.isFinite(Ie)?Ie:2,Be=W.get("mesh_sample_mode"),we=Be!==null?parseInt(Be,10):NaN,Ce=Number.isFinite(we)&&we>=0&&we<=7?we:1,Q=W.get("mesh_silhouette"),ee=Q==="1"||Q==="true",ie=W.get("mesh_invert_depth"),me=ie==="1"||ie==="true",Le=Math.max(1,window.devicePixelRatio||1),re={gaussian_scaling:1,sh_bias:.5,animate:N,animateMode:"presets",bg:{r:0,g:0,b:0,a:0},atlas_enabled:!1,bfc:Ee,bfc_cos:Re,mesh_cull:Z,mesh_margin:se,mesh_debug:oe,mesh_sample_mode:Ce,mesh_silhouette:ee,mesh_invert_depth:me,render_scale:1},_e=new ql.Pane({title:"Config",expanded:!0});_e.addInput(re,"animate",{label:"Animate"}).on("change",le=>{const M=N;N=le.value,k.enabled=!le.value,!M&&N&&ge.value&&ge.value.onAnimateStart(),M&&!N&&ge.value&&ge.value.onAnimateStop()}),_e.addInput(re,"animateMode",{label:"Anim path",options:{"Training views":"presets","Circle orbit":"circle"}});const ge={value:null};$c(h,le=>De(le,[],null,null),async(le,M,G)=>{let w=M??ue,i,f=null,d=null;const b=le.toLowerCase();if(b.endsWith(".bitymi")||b.includes(".bitymi?")){Is("downloading bundle ...");try{const{bundle:P}=await Fc(le,(r,m,u)=>{const p=r/1048576,a=m?m/(1024*1024):void 0,_=u/(1024*1024),y=m?Math.min(99,Math.floor(r/m*100)):void 0,v=a?`total ${a.toFixed(1)} MB`:"total -- MB",z=a&&y!==void 0?`${p.toFixed(1)} MB downloaded (${y}%)`:`${p.toFixed(1)} MB downloaded`,O=`${_.toFixed(2)} MB/s`;Gt(`downloading bundle ...
${v}, ${z}
