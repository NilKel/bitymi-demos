...
`),R="data:text/csv;charset=utf-8,"+encodeURIComponent(lt+z),U=document.createElement("a");U.href=R,U.download=`${this.downloadOnceFileName}.csv`,document.body.appendChild(U),U.click(),U.remove()}if(this.showPerfDialogNext){this.showPerfDialogNext=!1;try{alert($)}catch{console.warn("Unable to show dialog; metrics printed to console.")}}this.resultBuffer.unmap()}_setupTimestampQueries(){this.querySet=this.device.createQuerySet({type:"timestamp",count:this.totalQueryCount});const l=this.totalQueryCount*8;this.resolveBuffer=this.device.createBuffer({size:l,usage:GPUBufferUsage.QUERY_RESOLVE|GPUBufferUsage.COPY_SRC}),this.resultBuffer=this.device.createBuffer({size:l,usage:GPUBufferUsage.COPY_DST|GPUBufferUsage.MAP_READ})}_setupBuffers(){this.render_settings_buffer=this.device.createBuffer({label:"render settings",size:Xl,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST});const l=document.querySelector("canvas"),f=l?l.width:1,C=l?l.height:1;Yl({width:f,height:C,sh_bias:this.pc.sh_bias,color_K:this.pc.K,feature_mode:this.pc.feature_mode}),ti(this.device,this.render_settings_buffer),this.splat_2d_buffer=this.device.createBuffer({label:"splats_2d (Splat2DGS)",size:en(this.pc.num_points*Jl),usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST}),this.draw_indirect_buffer=this.device.createBuffer({label:"draw indirect",size:4*4,usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_DST|GPUBufferUsage.COPY_SRC|GPUBufferUsage.INDIRECT}),this.device.queue.writeBuffer(this.draw_indirect_buffer,0,new Uint32Array([4,0,0,0])),this.sh_solvers_buffer=this.device.createBuffer({label:"sh_solvers",size:en(this.pc.num_points*tc),usage:GPUBufferUsage.STORAGE|GPUBufferUsage.COPY_SRC|GPUBufferUsage.COPY_DST})}requestPerfDialog(){this.showPerfDialogNext=!0}requestDownloadMetrics(l){if(l&&l.trim().length>0){const f=l.trim().replace(/[^a-zA-Z0-9_\-]/g,"_");this.downloadOnceFileName=f.length>0?f:this.downloadOnceFileName}else{const f=new Date,C=`${f.getFullYear()}${String(f.getMonth()+1).padStart(2,"0")}${String(f.getDate()).padStart(2,"0")}_${String(f.getHours()).padStart(2,"0")}${String(f.getMinutes()).padStart(2,"0")}${String(f.getSeconds()).padStart(2,"0")}`;this.downloadOnceFileName=`fps_metrics_${C}`}this.downloadOnceNextRead=!0}requestReorder(){}async maybeReorderAfterSubmit(){}}function oc(o,l){return 2*Math.atan(l/(2*o))}function ac(o,l,f,C){const x=Math.tan(C/2),E=Math.tan(f/2),M=x*o,G=-M,A=E*o,L=-A,V=jt.create();return V[0]=2*o/(A-L),V[5]=-2*o/(M-G),V[2]=(A+L)/(A-L),V[6]=(M+G)/(M-G),V[14]=1,V[10]=l/(l-o),V[11]=-(l*o)/(l-o),jt.transpose(V,V),V}async function lc(o){ie(`loading scene camera file... : ${o}`);const f=await(await fetch(o)).json();return ie(`loaded cameras count: ${f.length}`),f.map(C=>{const x=I.clone(C.position),E=he.create(...C.rotation.flat()),M=E[0],G=E[4],A=E[8],L=E[1],V=E[5],q=E[9],Z=E[2],J=E[6],F=E[10];M*(V*F-q*J)-G*(L*F-q*Z)+A*(L*J-V*Z)<0&&(E[1]=-E[1],E[5]=-E[5],E[9]=-E[9]);const $=jt.fromMat3(E);return{position:x,rotation:$,img_name:C.img_name,id:C.id}})}const cc=4*2,uc=4*16,ei=4*uc+2*cc;function dc(o){return o.createBuffer({label:"camera uniform",size:ei,usage:GPUBufferUsage.UNIFORM|GPUBufferUsage.COPY_DST})}const re=new Float32Array(ei/Float32Array.BYTES_PER_ELEMENT);function pc(o,l,f,C,x,E){const M=new Float32Array(16),G=new Float32Array(16);jt.inverse(f,M),jt.inverse(C,G);const A=.5*x*C[0],L=.5*E*C[5];let V=0;re.set(f,V),V+=16,re.set(M,V),V+=16,re.set(C,V),V+=16,re.set(G,V),V+=16,re[V++]=x,re[V++]=E,re[V++]=A,re[V++]=L,o.queue.writeBuffer(l,0,re)}const In=class In{constructor(l,f){tt(this,"uniform_buffer");tt(this,"position",I.create());tt(this,"rotation",jt.create());tt(this,"fovY",45/180*Math.PI);tt(this,"fovX");tt(this,"focal",Ur.create());tt(this,"viewport",Ur.create());tt(this,"view_matrix",jt.identity());tt(this,"view_inv_matrix",jt.identity());tt(this,"proj_matrix",jt.identity());tt(this,"proj_inv_matrix",jt.identity());tt(this,"_negPos",I.create());tt(this,"look",I.create(0,0,1));tt(this,"up",I.create(0,1,0));tt(this,"right",I.create(1,0,0));this.canvas=l,this.device=f,this.uniform_buffer=dc(f),this.on_update_canvas()}on_update_canvas(){const l=.5*this.canvas.height/Math.tan(this.fovY*.5);this.focal[0]=l,this.focal[1]=l,this.fovX=oc(l,this.canvas.width),this.viewport[0]=this.canvas.width,this.viewport[1]=this.canvas.height,this.proj_matrix=ac(.01,100,this.fovX,this.fovY),jt.inverse(this.proj_matrix,this.proj_inv_matrix),this.update_buffer()}update_buffer(){this._negPos[0]=-this.position[0],this._negPos[1]=-this.position[1],this._negPos[2]=-this.position[2],jt.copy(this.rotation,this.view_matrix),jt.translate(this.view_matrix,this._negPos,this.view_matrix),jt.inverse(this.view_matrix,this.view_inv_matrix),I.transformMat4Upper3x3(In.Z_AXIS,this.view_inv_matrix,this.look),I.normalize(this.look,this.look),I.cross(this.up,this.look,this.right),I.normalize(this.right,this.right);let l=0;re.set(this.view_matrix,l),l+=16,re.set(this.view_inv_matrix,l),l+=16,re.set(this.proj_matrix,l),l+=16,re.set(this.proj_inv_matrix,l),l+=16,re.set(this.viewport,l),l+=2,re.set(this.focal,l),l+=2,this.device.queue.writeBuffer(this.uniform_buffer,0,re)}set_preset(l){I.copy(l.position,this.position),jt.copy(l.rotation,this.rotation),this.update_buffer()}setFov(l){this.fovY=l,this.on_update_canvas()}getFov(){return this.fovY}};tt(In,"Z_AXIS",I.create(0,0,1));let hs=In;const hc=I.create(1,0,0),fc=I.create(0,1,0);function _c(o,l){const f=o[0],C=o[4],x=o[8],E=o[1],M=o[5],G=o[9],A=o[2],L=o[6],V=o[10],q=f+M+V;let Z,J,F,O;if(q>0){const $=.5/Math.sqrt(q+1);Z=.25/$,J=(L-G)*$,F=(x-A)*$,O=(E-C)*$}else if(f>M&&f>V){const $=2*Math.sqrt(1+f-M-V);Z=(L-G)/$,J=.25*$,F=(C+E)/$,O=(x+A)/$}else if(M>V){const $=2*Math.sqrt(1+M-f-V);Z=(x-A)/$,J=(C+E)/$,F=.25*$,O=(G+L)/$}else{const $=2*Math.sqrt(1+V-f-M);Z=(E-C)/$,J=(x+A)/$,F=(G+L)/$,O=.25*$}return l[0]=J,l[1]=F,l[2]=O,l[3]=Z,l}class mc{constructor(l){tt(this,"element");tt(this,"enabled",!0);tt(this,"center",I.create(0,0,0));tt(this,"up",I.create(0,1,0));tt(this,"rotation",[0,0]);tt(this,"shift",[0,0]);tt(this,"scroll",0);tt(this,"speed",.1);tt(this,"sensitivity",.08);tt(this,"leftPressed",!1);tt(this,"rightPressed",!1);tt(this,"leftDragPans",!1);tt(this,"lastX",0);tt(this,"lastY",0);tt(this,"touches",new Map);tt(this,"lastTouchCenter",null);tt(this,"lastPinchDistance",null);tt(this,"lastTwoFingerAngle",null);tt(this,"lastTouchCount",0);tt(this,"roll",0);tt(this,"_dir",I.create());tt(this,"_right",I.create());tt(this,"_upCam",I.create());tt(this,"_scratch",I.create());tt(this,"_qY",se.create());tt(this,"_qX",se.create());tt(this,"_qRot",se.create());tt(this,"_qLocal",se.create());tt(this,"_qWorldToCam",se.create());tt(this,"_scratchMat3",he.create());tt(this,"bboxMin",null);tt(this,"bboxMax",null);tt(this,"anchor",I.create(0,0,0));tt(this,"downCallback",l=>{var f,C,x,E;if(this.enabled){if(l.pointerType==="touch"){this.touches.set(l.pointerId,{x:l.pageX,y:l.pageY}),this.handleTouchGestures(),(C=(f=l.target)==null?void 0:f.setPointerCapture)==null||C.call(f,l.pointerId),l.preventDefault();return}l.isPrimary&&(l.button===0?(this.leftPressed=!0,this.leftDragPans=l.shiftKey):l.button===2?this.rightPressed=!0:this.rightPressed=!0,this.lastX=l.pageX,this.lastY=l.pageY,(E=(x=l.target)==null?void 0:x.setPointerCapture)==null||E.call(x,l.pointerId),l.preventDefault())}});tt(this,"moveCallback",l=>{if(!this.enabled)return;if(l.pointerType==="touch"){if(!this.touches.has(l.pointerId))return;this.touches.set(l.pointerId,{x:l.pageX,y:l.pageY}),this.handleTouchGestures(),l.preventDefault();return}if(!l.isPrimary||!this.leftPressed&&!this.rightPressed)return;l.preventDefault();const f=l.pageX-this.lastX,C=l.pageY-this.lastY;this.lastX=l.pageX,this.lastY=l.pageY,this.leftPressed&&!this.leftDragPans?(this.rotation[0]+=f,this.rotation[1]-=C):(this.rightPressed||this.leftPressed&&this.leftDragPans)&&(this.shift[1]-=f,this.shift[0]+=C)});tt(this,"upCallback",l=>{var f,C,x,E;if(l.pointerType==="touch"){this.touches.delete(l.pointerId),this.handleTouchGestures(),(C=(f=l.target)==null?void 0:f.releasePointerCapture)==null||C.call(f,l.pointerId),l.preventDefault();return}l.button===0?this.leftPressed=!1:l.button===2?this.rightPressed=!1:this.rightPressed=!1,(E=(x=l.target)==null?void 0:x.releasePointerCapture)==null||E.call(x,l.pointerId),l.preventDefault()});tt(this,"wheelCallback",l=>{if(!this.enabled||(l.preventDefault(),this.rightPressed))return;let f=l.deltaY;l.deltaMode===1?f*=16:l.deltaMode===2&&(f*=100),this.scroll+=f*.01});this.camera=l,this.registerElement(l.canvas)}registerElement(l){this.element&&this.element!==l&&(this.element.removeEventListener("pointerdown",this.downCallback),this.element.removeEventListener("pointermove",this.moveCallback),this.element.removeEventListener("pointerup",this.upCallback),this.element.removeEventListener("wheel",this.wheelCallback)),this.element=l,this.element.addEventListener("pointerdown",this.downCallback),this.element.addEventListener("pointermove",this.moveCallback),this.element.addEventListener("pointerup",this.upCallback),this.element.addEventListener("wheel",this.wheelCallback,{passive:!1}),this.element.addEventListener("contextmenu",f=>f.preventDefault())}setCenter(l){I.copy(l,this.center),I.copy(l,this.anchor)}setOrbitPivot(l){I.set(l[0],l[1],l[2],this.center),this._reorientCameraToCenter()}setOrbitDepth(l){if(!isFinite(l)||l<.001)return;const f=this.camera.rotation;I.set(f[2],f[6],f[10],this._dir),I.normalize(this._dir,this._dir),I.scale(this._dir,l,this._dir),I.add(this.camera.position,this._dir,this.center)}_reorientCameraToCenter(){const l=this.camera;if(I.subtract(this.center,l.position,this._scratch),I.length(this._scratch)<1e-6)return;I.normalize(this._scratch,this._scratch),I.cross(this.up,this._scratch,this._right),I.length(this._right)<1e-6&&I.set(1,0,0,this._right),I.normalize(this._right,this._right),I.cross(this._scratch,this._right,this._upCam),I.normalize(this._upCam,this._upCam);const f=l.rotation;f[0]=this._right[0],f[1]=this._upCam[0],f[2]=this._scratch[0],f[3]=0,f[4]=this._right[1],f[5]=this._upCam[1],f[6]=this._scratch[1],f[7]=0,f[8]=this._right[2],f[9]=this._upCam[2],f[10]=this._scratch[2],f[11]=0,f[12]=0,f[13]=0,f[14]=0,f[15]=1,l.update_buffer()}setBbox(l,f){this.bboxMin=I.create(l[0],l[1],l[2]),this.bboxMax=I.create(f[0],f[1],f[2]);const C=(l[0]+f[0])*.5,x=(l[1]+f[1])*.5,E=(l[2]+f[2])*.5;I.set(C,x,E,this.center),I.set(C,x,E,this.anchor)}resetToCamera(){const l=this.camera.rotation;I.set(l[2],l[6],l[10],this._dir),I.normalize(this._dir,this._dir);let f=null;if(this.bboxMin&&this.bboxMax){let C=-1/0,x=1/0,E=!1;for(let M=0;M<3;M++){const G=this._dir[M],A=this.bboxMin[M]-this.camera.position[M],L=this.bboxMax[M]-this.camera.position[M];if(Math.abs(G)>1e-8){const V=A/G,q=L/G;C=Math.max(C,Math.min(V,q)),x=Math.min(x,Math.max(V,q))}else if(A>0||L<0){E=!0;break}}!E&&C<=x&&x>0&&(f=(Math.max(C,0)+x)*.5)}if(f===null||!isFinite(f)||f<.001){I.subtract(this.anchor,this.camera.position,this._scratch);const C=I.dot(this._scratch,this._dir);f=C>.001?C:I.length(this._scratch)}f=Math.max(.1,f),I.scale(this._dir,f,this._dir),I.add(this.camera.position,this._dir,this.center)}handleTouchGestures(){const l=this.touches.size;if(l!==this.lastTouchCount&&(this.lastTouchCenter=null,this.lastPinchDistance=null,this.lastTwoFingerAngle=null),this.lastTouchCount=l,l===1){const f=this.touches.values().next().value;if(this.lastTouchCenter){const C=f.x-this.lastTouchCenter[0],x=f.y-this.lastTouchCenter[1];this.rotation[0]+=C*.3,this.rotation[1]-=x*.3}this.lastTouchCenter=[f.x,f.y]}else if(l===2){const f=Array.from(this.touches.values()),C=(f[0].x+f[1].x)*.5,x=(f[0].y+f[1].y)*.5,E=f[1].x-f[0].x,M=f[1].y-f[0].y,G=Math.hypot(E,M),A=Math.atan2(M,E);if(this.lastTouchCenter!==null&&this.lastPinchDistance!==null&&this.lastTwoFingerAngle!==null){const L=C-this.lastTouchCenter[0],V=x-this.lastTouchCenter[1],q=Math.hypot(L,V),Z=Math.abs(G-this.lastPinchDistance);let J=A-this.lastTwoFingerAngle;J>Math.PI&&(J-=2*Math.PI),J<-Math.PI&&(J+=2*Math.PI),q>.5&&(this.shift[1]-=L,this.shift[0]+=V),Z>1&&this.lastPinchDistance>.001&&(this.scroll+=-Math.log(G/this.lastPinchDistance)*10),Math.abs(J)>.0087&&(this.roll+=-J)}this.lastTouchCenter=[C,x],this.lastPinchDistance=G,this.lastTwoFingerAngle=A}}update(l){if(!this.enabled||Math.abs(this.rotation[0])<1e-4&&Math.abs(this.rotation[1])<1e-4&&Math.abs(this.shift[0])<1e-4&&Math.abs(this.shift[1])<1e-4&&Math.abs(this.scroll)<1e-4&&Math.abs(this.roll)<1e-4)return;const f=this.camera;{const O=f.rotation;this.up[0]=O[1],this.up[1]=O[5],this.up[2]=O[9],I.length(this.up)>1e-6?I.normalize(this.up,this.up):I.set(0,1,0,this.up)}let C=!1;if(Math.abs(this.roll)>1e-4){const O=f.rotation;I.set(O[2],O[6],O[10],this._scratch),I.normalize(this._scratch,this._scratch),se.fromAxisAngle(this._scratch,this.roll,this._qRot),I.transformQuat(this.up,this._qRot,this.up),I.normalize(this.up,this.up),this.roll=0,C=!0}I.subtract(f.position,this.center,this._dir);let x=I.length(this._dir);x<1e-6&&(x=1e-6);const E=Math.exp(Math.log(x)+this.scroll*l*10*this.speed);I.scale(this._dir,E/x,this._dir),x=E;const M=f.rotation;this._right[0]=M[0],this._right[1]=M[4],this._right[2]=M[8],I.normalize(this._right,this._right),I.length(this._right)<1e-6&&I.set(1,0,0,this._right);const G=I.create(M[1],M[5],M[9]);I.normalize(G,G),I.length(G)<1e-6&&I.set(0,1,0,G);const A=l*this.speed*.1*x,L=this.shift[1]*A,V=-this.shift[0]*A;I.scale(this._right,L,this._scratch),I.add(this.center,this._scratch,this.center),I.add(f.position,this._scratch,f.position),I.scale(G,V,this._scratch),I.add(this.center,this._scratch,this.center),I.add(f.position,this._scratch,f.position);const q=this.rotation[0]*l*this.sensitivity,Z=this.rotation[1]*l*this.sensitivity;if(Math.abs(q)>1e-5||Math.abs(Z)>1e-5||C){const O=f.rotation;_c(O,this._qWorldToCam),se.fromAxisAngle(hc,-Z,this._qX),se.fromAxisAngle(fc,-q,this._qY),se.multiply(this._qX,this._qY,this._qLocal),se.normalize(this._qLocal,this._qLocal),se.multiply(this._qLocal,this._qWorldToCam,this._qWorldToCam),se.normalize(this._qWorldToCam,this._qWorldToCam),he.fromQuat(this._qWorldToCam,this._scratchMat3),jt.fromMat3(this._scratchMat3,f.rotation);const $=f.rotation,lt=$[2],z=$[6],R=$[10];f.position[0]=this.center[0]-lt*x,f.position[1]=this.center[1]-z*x,f.position[2]=this.center[2]-R*x,this.up[0]=$[1],this.up[1]=$[5],this.up[2]=$[9],I.normalize(this.up,this.up)}else I.add(this.center,this._dir,f.position);f.update_buffer();const F=Math.pow(.8,l*60);this.rotation[0]*=F,Math.abs(this.rotation[0])<1e-4&&(this.rotation[0]=0),this.rotation[1]*=F,Math.abs(this.rotation[1])<1e-4&&(this.rotation[1]=0),this.shift[0]*=F,Math.abs(this.shift[0])<1e-4&&(this.shift[0]=0),this.shift[1]*=F,Math.abs(this.shift[1])<1e-4&&(this.shift[1]=0),this.scroll*=F,Math.abs(this.scroll)<1e-4&&(this.scroll=0)}}function ni(o){const l=I.create();for(const f of o)I.add(l,f,l);return I.scale(l,1/Math.max(o.length,1),l)}function si(o,l){const f=he.create();he.inverse(o,f);const C=I.create();return C[0]=f[0]*l[0]+f[4]*l[1]+f[8]*l[2],C[1]=f[1]*l[0]+f[5]*l[1]+f[9]*l[2],C[2]=f[2]*l[0]+f[6]*l[1]+f[10]*l[2],C}function bc(o){const l=o.slice(),f=[1,0,0,0,1,0,0,0,1],C=(A,L)=>l[A*3+L],x=(A,L,V)=>{l[A*3+L]=V},E=(A,L)=>f[A*3+L],M=(A,L,V)=>{f[A*3+L]=V};for(let A=0;A<30;A++){let L=0,V=1,q=Math.abs(C(0,1));if(Math.abs(C(0,2))>q&&(L=0,V=2,q=Math.abs(C(0,2))),Math.abs(C(1,2))>q&&(L=1,V=2,q=Math.abs(C(1,2))),q<1e-12)break;const Z=C(L,L),J=C(V,V),F=C(L,V);let O;Math.abs(Z-J)<1e-30?O=Math.PI/4*Math.sign(F):O=.5*Math.atan2(2*F,Z-J);const $=Math.cos(O),lt=Math.sin(O);for(let z=0;z<3;z++){const R=C(z,L),U=C(z,V);x(z,L,$*R+lt*U),x(z,V,-lt*R+$*U)}for(let z=0;z<3;z++){const R=C(L,z),U=C(V,z);x(L,z,$*R+lt*U),x(V,z,-lt*R+$*U)}for(let z=0;z<3;z++){const R=E(z,L),U=E(z,V);M(z,L,$*R+lt*U),M(z,V,-lt*R+$*U)}}const G=[];for(let A=0;A<3;A++)G.push({val:C(A,A),vec:I.create(E(0,A),E(1,A),E(2,A))});return G.sort((A,L)=>L.val-A.val),{vals:[G[0].val,G[1].val,G[2].val],vecs:[G[0].vec,G[1].vec,G[2].vec]}}function vc(o,l){const f=ni(o);let C=0,x=0,E=0,M=0,G=0,A=0;for(const F of o){const O=F[0]-f[0],$=F[1]-f[1],lt=F[2]-f[2];C+=O*O,x+=O*$,E+=O*lt,M+=$*$,G+=$*lt,A+=lt*lt}const L=[C,x,E,x,M,G,E,G,A],{vecs:V}=bc(L);let q=V[0],Z=V[1],J=V[2];return I.dot(J,l)<0&&(I.scale(J,-1,J),I.scale(Z,-1,Z)),{centroid:f,normal:J,u:q,v:Z}}function gc(o){let l=0,f=0,C=0,x=0,E=0,M=0,G=0,A=0,L=0;for(const[$,lt]of o){const z=-2*$,R=-2*lt,U=1,ct=-($*$+lt*lt);l+=z*z,f+=z*R,C+=z*U,x+=R*R,E+=R*U,M+=U*U,G+=z*ct,A+=R*ct,L+=U*ct}const V=he.create(l,f,C,f,x,E,C,E,M),q=si(V,I.create(G,A,L)),Z=q[0],J=q[1],F=q[2],O=Z*Z+J*J-F;return{center:[Z,J],radius:Math.sqrt(Math.max(O,1e-12))}}function wc(o,l){let f=0,C=0,x=0,E=0,M=0,G=0,A=0,L=0,V=0;for(let Z=0;Z<o.length;Z++){const J=o[Z],F=I.normalize(l[Z],I.create()),O=1-F[0]*F[0],$=-F[0]*F[1],lt=-F[0]*F[2],z=1-F[1]*F[1],R=-F[1]*F[2],U=1-F[2]*F[2];f+=O,C+=$,x+=lt,E+=z,M+=R,G+=U,A+=O*J[0]+$*J[1]+lt*J[2],L+=$*J[0]+z*J[1]+R*J[2],V+=lt*J[0]+R*J[1]+U*J[2]}const q=he.create(f,C,x,C,E,M,x,M,G);return si(q,I.create(A,L,V))}function ri(o,l={}){if(o.length===0)return null;const f=l.tiltDownDeg??8,C=l.radiusScale??1,x=l.alignFirst??!0,E=(l.direction??"ccw")==="ccw"?1:-1,M=o.map(Y=>I.clone(Y.position)),G=o.map(Y=>{const et=Y.rotation;return I.create(et[8],et[9],et[10])}),A=o.map(Y=>{const et=Y.rotation;return I.create(et[4],et[5],et[6])}),L=ni(A),V=I.normalize(I.scale(L,-1,I.create())),{centroid:q,normal:Z,u:J,v:F}=vc(M,V),O=M.map(Y=>{const et=I.sub(Y,q,I.create());return[I.dot(et,J),I.dot(et,F)]}),{center:$,radius:lt}=gc(O),z=lt*C,R=I.add(q,I.add(I.scale(J,$[0],I.create()),I.scale(F,$[1],I.create()),I.create()),I.create()),U=wc(M,G),ct=z*Math.tan(f*Math.PI/180),K=I.sub(U,I.scale(Z,ct,I.create()),I.create());let Q=0;if(x){const Y=I.sub(M[0],R,I.create());Q=Math.atan2(I.dot(Y,F),I.dot(Y,J))/(2*Math.PI)%1,Q<0&&(Q+=1)}return console.log(`[orbit] fit ${o.length} train cams: radius=${z.toFixed(2)}, tilt=${f}°, normal=[${Z[0].toFixed(2)}, ${Z[1].toFixed(2)}, ${Z[2].toFixed(2)}], startPhase=${Q.toFixed(3)}`),{center:R,radius:z,normal:Z,u:J,v:F,lookAt:K,startPhase:Q,direction:E}}function ii(o,l){const f=(o.startPhase+l*o.direction)*2*Math.PI,C=Math.cos(f),x=Math.sin(f),E=I.add(o.center,I.add(I.scale(o.u,o.radius*C,I.create()),I.scale(o.v,o.radius*x,I.create()),I.create()),I.create()),M=I.normalize(I.sub(o.lookAt,E,I.create())),G=I.cross(M,o.normal,I.create());I.length(G)<1e-6&&I.copy(o.u,G),I.normalize(G,G);const A=I.cross(M,G,I.create());I.normalize(A,A);const L=jt.create();return L[0]=G[0],L[1]=A[0],L[2]=M[0],L[3]=0,L[4]=G[1],L[5]=A[1],L[6]=M[1],L[7]=0,L[8]=G[2],L[9]=A[2],L[10]=M[2],L[11]=0,L[12]=0,L[13]=0,L[14]=0,L[15]=1,{position:E,rotation:L,img_name:`orbit_${(l*1e3).toFixed(0)}`,id:0}}function xc(o,l={}){const f=ri(o,l);if(!f)return[];const C=l.numViews??120;return Array.from({length:C},(x,E)=>({...ii(f,E/C),img_name:`circle_${E.toString().padStart(4,"0")}`,id:E}))}async function yc(){if(!("xr"in navigator)||!navigator.xr)return!1;try{return await navigator.xr.isSessionSupported("immersive-vr")}catch{return!1}}async function Pc(o){const l=window.XRGPUBinding??window.XRWebGPUBinding;if(!l)throw new Error("XRGPUBinding is not available in this browser. On Quest Browser you may need to enable it under chrome://flags/#webxr-webgpu-binding (inspect via ADB). Falling back to a WebGL2 XR layer is a future TODO.");const f=await navigator.xr.requestSession("immersive-vr",{requiredFeatures:["local"],optionalFeatures:["local-floor","bounded-floor","hand-tracking"]}),C=new l(f,o.device),x=C.createProjectionLayer({colorFormat:"rgba8unorm-srgb",depthStencilFormat:"depth24plus",scaleFactor:1});f.updateRenderState({layers:[x]});const E=await f.requestReferenceSpace("local"),M=I.create(0,0,-1.5),G=jt.identity(),A=jt.create(),L=jt.create(),V=new Promise(Z=>{f.addEventListener("end",()=>{o.onExit(),Z()})}),q=performance.now();f.requestAnimationFrame(function Z(J,F){f.requestAnimationFrame(Z);const O=F.getViewerPose(E);if(!O)return;Sc(f,F,E,M,G),jt.copy(G,A);const $=I.create(-M[0],-M[1],-M[2]);jt.translate(A,$,A);const lt=o.device.createCommandEncoder({label:"xr-frame"});for(const z of O.views){const R=C.getViewSubImage(x,z),U=R.colorTexture.createView(),ct=R.viewport,K=z.transform.inverse.matrix;jt.multiply(K,A,L),pc(o.device,o.camera.uniform_buffer,L,z.projectionMatrix,ct.width,ct.height),o.renderer.frame(lt,U,!1)}o.device.queue.submit([lt.finish()])}),console.log(`[XR] session started in ${(performance.now()-q).toFixed(1)} ms; ${f.inputSources.length} controllers`),await V}function Sc(o,l,f,C,x){const M=.013888888888888888,G=1.5/72,A=.8/72;for(const L of o.inputSources){const V=L.gamepad;if(!V||V.axes.length<2)continue;const q=V.axes[2]??V.axes[0],Z=V.axes[3]??V.axes[1];if(!(Math.abs(q)<.15&&Math.abs(Z)<.15)){if(L.handedness==="left"){const J=Math.abs(q)>.15?q*M:0,F=Math.abs(Z)>.15?-Z*M:0;C[0]+=J,C[1]+=F}else if(L.handedness==="right"&&(Math.abs(Z)>.15&&(C[2]+=Z*G),Math.abs(q)>.15)){const J=-q*A,F=jt.rotateY(jt.identity(),J);jt.multiply(F,x,x)}}}}const oi="BITYMI01",kc=0,Cc=1,Ec=2,Mc=3,Bc=4,Dc=5;function An(o){const l=(o&32768)>>15,f=(o&31744)>>10,C=o&1023;return f===0?(l?-1:1)*Math.pow(2,-14)*(C/1024):f===31?C?NaN:l?-1/0:1/0:(l?-1:1)*Math.pow(2,f-15)*(1+C/1024)}function Wr(o,l,f,C,x,E){const M=(o-f.width*.5)/C.focal[0],G=-((l-f.height*.5)/C.focal[1]),A=C.rotation,L=A[0],V=A[4],q=A[8],Z=A[1],J=A[5],F=A[9],O=A[2],$=A[6],lt=A[10];let z=M*L+G*Z+O,R=M*V+G*J+$,U=M*q+G*F+lt;const ct=Math.hypot(z,R,U)||1;z/=ct,R/=ct,U/=ct;const K=C.position[0],Q=C.position[1],Y=C.position[2],et=E*.005,_t=et*et,Vt=new Uint32Array(x.buffer,x.byteOffset,x.length),Rt=x.length/8;let Tt=1/0,pt=0,kt=0,wt=0,H=-1;for(let st=0;st<Rt;st++){const It=st*8,Mt=x[It+0]-K,zt=x[It+1]-Q,Dt=x[It+2]-Y,Ot=Mt*z+zt*R+Dt*U;if(Ot<=0)continue;const xt=Mt-Ot*z,ht=zt-Ot*R,Wt=Dt-Ot*U;if(xt*xt+ht*ht+Wt*Wt>=_t||!(Vt[It+7]>>>16&1))continue;const g=Vt[It+5],i=Vt[It+6],h=An(g&65535),d=An(g>>>16&65535),b=An(i&65535),y=An(i>>>16&65535),S=Math.hypot(h,d,b,y)||1,k=h/S,r=d/S,m=b/S,u=y/S,p=2*(r*u+k*m),a=2*(m*u-k*r),_=1-2*(r*r+m*m),w=z*p+R*a+U*_;let v;Math.abs(w)>1e-6?(v=(Mt*p+zt*a+Dt*_)/w,(!isFinite(v)||v<=0)&&(v=Ot)):v=Ot,v<Tt&&(Tt=v,pt=K+v*z,kt=Q+v*R,wt=Y+v*U,H=st)}return H<0?null:[pt,kt,wt]}function ai(o){const l=new Uint8Array(o),f=new TextDecoder().decode(l.subarray(0,8));if(f!==oi)throw new Error(`Not a BITYMI bundle (bad magic '${f}')`);const C=new DataView(o),x=C.getUint32(8,!0),E=12,M=20;let G=null,A=null,L=null;for(let V=0;V<x;V++){const q=E+V*M,Z=C.getUint32(q+0,!0),J=Number(C.getBigUint64(q+4,!0)),F=Number(C.getBigUint64(q+12,!0)),O=l.slice(J,J+F).buffer;Z===kc||Z===Cc||Z===Dc?G=O:Z===Ec?A=O:(Z===Mc||Z===Bc)&&(L=O)}if(G===null)throw new Error("BITYMI bundle has no point cloud chunk");return{pcBuffer:G,camerasBuffer:A,atlasBuffer:L}}async function Tc(o,l){var M;const f=await fetch(o);if(!f.ok)throw new Error(`fetch failed: ${f.status} ${f.statusText}`);const C=(()=>{const G=f.headers.get("content-length");return G&&parseInt(G,10)||void 0})(),x=(M=f.body)==null?void 0:M.getReader();let E;if(!x)E=await f.arrayBuffer(),l&&l(E.byteLength,C,0);else{const G=[];let A=0,L=performance.now(),V=0;for(;;){const{done:J,value:F}=await x.read();if(J)break;G.push(F),A+=F.byteLength;const O=performance.now();if(O-L>=150&&l){const $=(A-V)/((O-L)/1e3);l(A,C,$),L=O,V=A}}const q=new Uint8Array(A);let Z=0;for(const J of G)q.set(J,Z),Z+=J.byteLength;E=q.buffer,l&&l(A,C,0)}return E.byteLength>=8&&new TextDecoder().decode(new Uint8Array(E,0,8))===oi?{bundle:ai(E),rawPly:null}:{bundle:null,rawPly:E}}function Ac(o){return new Promise(l=>{const f=document.createElement("input");f.type="file",f.accept=o,f.style.display="none",f.onchange=()=>{var C;return l(((C=f.files)==null?void 0:C[0])??null)},document.body.appendChild(f),f.click(),setTimeout(()=>document.body.removeChild(f),1e3)})}function Rc(o,l,f){const C=document.getElementById("ui-panel-container"),x=document.getElementById("load-button"),E=document.getElementById("quick-links");x&&(x.onclick=async()=>{const L=await Ac(".ply,.bitymi");if(L)if(C&&(C.style.display="none"),L.name.toLowerCase().endsWith(".bitymi")){const V=await L.arrayBuffer(),{pcBuffer:q}=ai(V),Z=new File([q],L.name.replace(/\.bitymi$/i,".ply"),{type:"application/octet-stream"}),J=await cs(Z,o);l(J)}else{const V=await cs(L,o);l(V)}}),E&&(E.innerHTML="");const M=new URLSearchParams(window.location.search),G=M.get("bundle")??M.get("model_url"),A=M.get("camera_url");G&&(C&&(C.style.display="none"),f(G,A))}async function Ic(o,l,f,C){const x=new hs(o,f),E=new mc(x);let M=!1;o.addEventListener("pointerdown",()=>{M=!0}),window.addEventListener("pointerup",()=>{M=!1});const G="rgba8unorm";l.configure({device:f,format:G,alphaMode:"opaque",usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.STORAGE_BINDING});let A=null;const L=()=>{x.on_update_canvas(),A!==null&&vn(o.width,o.height,f,A.render_settings_buffer)};new ResizeObserver(()=>{const Q=Math.max(.25,R.render_scale),Y=Math.max(1,Math.ceil(Q*o.clientWidth)),et=Math.max(1,Math.ceil(Q*o.clientHeight));o.width===Y&&o.height===et||(o.width=Y,o.height=et,L())}).observe(o);let q=0,Z=0;const J=()=>{(o.width!==q||o.height!==Z)&&(q=o.width,Z=o.height,L())},F=new URLSearchParams(window.location.search);let $=F.get("animation")==="1";E.enabled=!$;const lt=F.get("camera_url"),z=Math.max(1,window.devicePixelRatio||1),R={gaussian_scaling:1,sh_bias:.5,animate:$,animateMode:"presets",bg:{r:0,g:0,b:0,a:0},atlas_enabled:!1,bfc:!1,render_scale:1},U=new Ol.Pane({title:"Config",expanded:!0});U.addInput(R,"animate",{label:"Animate"}).on("change",Q=>{const Y=$;$=Q.value,E.enabled=!Q.value,!Y&&$&&ct.value&&ct.value.onAnimateStart(),Y&&!$&&ct.value&&ct.value.onAnimateStop()}),U.addInput(R,"animateMode",{label:"Anim path",options:{"Training views":"presets","Circle orbit":"circle"}});const ct={value:null};Rc(f,Q=>K(Q,[],null),async(Q,Y)=>{let et=Y??lt,_t,Vt=null;const Rt=Q.toLowerCase();if(Rt.endsWith(".bitymi")||Rt.includes(".bitymi?")){_s("downloading bundle ...");try{const{bundle:pt}=await Tc(Q,(wt,H,st)=>{const It=wt/1048576,Mt=H?H/(1024*1024):void 0,zt=st/(1024*1024),Dt=H?Math.min(99,Math.floor(wt/H*100)):void 0,Ot=Mt?`total ${Mt.toFixed(1)} MB`:"total -- MB",xt=Mt&&Dt!==void 0?`${It.toFixed(1)} MB downloaded (${Dt}%)`:`${It.toFixed(1)} MB downloaded`,ht=`${zt.toFixed(2)} MB/s`;Ne(`downloading bundle ...
${Ot}, ${xt}
${ht}`)});if(!pt)throw new Error("Expected a .bitymi bundle");Ne("parsing PLY ...");const kt=new File([pt.pcBuffer],"bundle.ply",{type:"application/octet-stream"});if(_t=await cs(kt,f),!et&&pt.camerasBuffer&&(et=URL.createObjectURL(new Blob([pt.camerasBuffer],{type:"application/json"}))),pt.atlasBuffer){const wt=pt.atlasBuffer.byteLength/1048576;Ne(`uploading atlas ...
${wt.toFixed(1)} MB BC7`);try{const H=Rl(pt.atlasBuffer);Vt=Vl(f,H,!0)}catch(H){console.warn("[atlas] failed to parse/upload atlas:",H)}}}catch(pt){throw gn(),pt}}else _t=await Gl(Q,f);const Tt=et?await lc(et):[];Tt.length>0&&x.set_preset(Tt[0]),K(_t,Tt,Vt)});function K(Q,Y=[],et=null){const _t=[(Q.bbox.min[0]+Q.bbox.max[0])/2,(Q.bbox.min[1]+Q.bbox.max[1])/2,(Q.bbox.min[2]+Q.bbox.max[2])/2];E.setBbox(Q.bbox.min,Q.bbox.max);const Vt=.5*Math.sqrt((Q.bbox.max[0]-Q.bbox.min[0])**2+(Q.bbox.max[1]-Q.bbox.min[1])**2+(Q.bbox.max[2]-Q.bbox.min[2])**2);function Rt(X,at){const it=Wr(X,at,o,x,Q.surfel_data,Vt);it&&(E.setOrbitPivot(it),console.log(`[pick] orbit pivot → (${it[0].toFixed(3)}, ${it[1].toFixed(3)}, ${it[2].toFixed(3)})`))}function Tt(){const X=o.width*.5,at=o.height*.5,it=Wr(X,at,o,x,Q.surfel_data,Vt);if(!it)return;const gt=x.rotation,Pt=gt[2],Et=gt[6],mt=gt[10],vt=it[0]-x.position[0],Gt=it[1]-x.position[1],Ut=it[2]-x.position[2],qt=vt*Pt+Gt*Et+Ut*mt;qt>0&&E.setOrbitDepth(qt)}if(Y.length===0){const X=Q.bbox.max[0]-Q.bbox.min[0],at=Q.bbox.max[1]-Q.bbox.min[1],it=Q.bbox.max[2]-Q.bbox.min[2],Pt=.5*Math.sqrt(X*X+at*at+it*it)*.5;I.set(_t[0]-Pt,_t[1]-Pt,_t[2]-Pt,x.position);const Et=I.create(Pt,Pt,Pt);I.normalize(Et,Et);const mt=I.create(0,1,0),vt=I.create();I.cross(mt,Et,vt),I.normalize(vt,vt);const Gt=I.create();I.cross(Et,vt,Gt);const Ut=he.create(vt[0],Gt[0],Et[0],vt[1],Gt[1],Et[1],vt[2],Gt[2],Et[2]);jt.fromMat3(Ut,x.rotation),x.update_buffer()}E.setCenter(I.create(Q.centroid[0],Q.centroid[1],Q.centroid[2]));const pt=new ic(Q,f,G,x.uniform_buffer,C,et);A=pt;let kt=!1;(async()=>{if(!await yc())return;const X=document.getElementById("enter-vr-button"),at=async()=>{if(!kt){kt=!0,X&&(X.disabled=!0,X.textContent="🥽 In VR…");try{await Pc({device:f,renderer:pt,camera:x,onExit:()=>{kt=!1,X&&(X.disabled=!1,X.textContent="🥽 Enter VR"),vn(o.width,o.height,f,pt.render_settings_buffer),x.on_update_canvas(),o.addEventListener("pointerdown",at,{once:!0})}})}catch(it){console.error("[XR] session failed:",it),kt=!1,X&&(X.disabled=!1,X.textContent="🥽 Enter VR"),o.addEventListener("pointerdown",at,{once:!0})}}};X&&(X.style.display="",X.addEventListener("click",at)),o.addEventListener("pointerdown",at,{once:!0})})(),window.__xrActive=()=>kt,vn(o.width,o.height,f,pt.render_settings_buffer),R.atlas_enabled=et!==null;let wt=!1;const H=(()=>{if(et!==null)return`${et.meta.format===2?"BC7":et.meta.format===3?"ASTC 4×4":et.meta.format===5?"RVQ-paired (typeA)":et.meta.format===6?"RVQ-paired + BC7 codebook (typeB)":et.meta.format===7?"BC7 codebook gather (typeD)":`format=${et.meta.format}`} ${et.meta.width}×${et.meta.height}, ${et.meta.n_layers} layers`;const X=f.features.has("texture-compression-bc"),at=f.features.has("texture-compression-astc");return`no atlas in bundle (GPU supports: ${(X?["BC7"]:[]).concat(at?["ASTC"]:[]).join("+")||"none"})`})();console.log("[atlas]",H),qr(Q.sh_bias,f,pt.render_settings_buffer),Fr(R.gaussian_scaling,f,pt.render_settings_buffer),R.sh_bias=Q.sh_bias;const st=Q.num_points.toLocaleString(),It={stats:`${st} surfels · -- fps`};U.addMonitor(It,"stats",{label:"Stats",interval:200}),U.addMonitor({atlas:H},"atlas",{label:"Atlas"});const Mt={stages:"— ms · awaiting timestamp data"};U.addMonitor(Mt,"stages",{label:"Stages",interval:500});let zt=null;if(pt.timeQueryEnabled){zt=document.createElement("canvas"),zt.width=280,zt.height=24,zt.style.cssText="position:fixed;right:8px;bottom:8px;width:280px;height:24px;background:#0008;border:1px solid #2a2a2a;border-radius:4px;font-family:ui-monospace,Menlo,monospace;pointer-events:none;z-index:1000;",document.body.appendChild(zt);const X=it=>{const gt=zt.getContext("2d");if(!gt)return;const Pt=zt.width,Et=zt.height;if(gt.clearRect(0,0,Pt,Et),!it||it.total<=0){gt.fillStyle="#888",gt.font="11px ui-monospace,Menlo,monospace",gt.fillText("awaiting GPU timestamps…",8,16);return}const mt=[{label:"cull",ms:it.cull,color:"#3aa3ff"},{label:"pre",ms:it.preprocess,color:"#3ad27a"},{label:"sort",ms:it.sort,color:"#ffa53a"},{label:"render",ms:it.render,color:"#ff5566"}],vt=Math.max(.001,mt.reduce((Ut,qt)=>Ut+qt.ms,0));let Gt=0;gt.font="10px ui-monospace,Menlo,monospace",gt.textBaseline="middle";for(const Ut of mt){const qt=Ut.ms/vt*Pt;gt.fillStyle=Ut.color,gt.fillRect(Gt,0,qt,Et),qt>=38&&(gt.fillStyle="#000c",gt.fillText(`${Ut.label} ${Ut.ms.toFixed(1)}`,Gt+4,Et/2)),Gt+=qt}};setInterval(()=>{wt||pt.readPerfMetrics({silent:!0}).then(()=>{const it=pt.lastStageBreakdownMs;X(it),it&&(Mt.stages=`${it.total.toFixed(1)} ms · cull ${it.cull.toFixed(1)} / pre ${it.preprocess.toFixed(1)} / sort ${it.sort.toFixed(1)} / render ${it.render.toFixed(1)}`)}).catch(it=>console.warn("[perf] readPerfMetrics failed:",it))},500)}const Dt=.4,Ot=3,xt=.3;let ht=null,Wt=0,B=0;const j=se.create(),g=he.create();let i=Y.length>0?0:-1;const h={view:Y.length>0?`${i+1} / ${Y.length}: ${Y[i].img_name??i}`:"— no presets —"};U.addMonitor(h,"view",{label:"View",interval:100});function d(X){const at=he.create(X[0],X[1],X[2],X[4],X[5],X[6],X[8],X[9],X[10]);return se.fromMat(at)}function b(X,at){ht={fromPos:I.clone(x.position),toPos:I.clone(X.position),fromQuat:se.normalize(d(x.rotation)),toQuat:se.normalize(d(X.rotation)),target:X,t:0,duration:Math.max(.01,at)}}const y=(X,at=!0)=>{if(Y.length===0)return;i=(X%Y.length+Y.length)%Y.length;const it=Y[i];at?b(it,Dt):(x.set_preset(it),E.resetToCamera(),Tt()),h.view=`${i+1} / ${Y.length}: ${Y[i].img_name??i}`};Y.length>0&&(U.addButton({title:"◀ Prev view"}).on("click",()=>y(i-1)),U.addButton({title:"Next view ▶"}).on("click",()=>y(i+1)));const S=Y.length>0?ri(Y,{tiltDownDeg:15,alignFirst:!0}):null,k=S?xc(Y,{numViews:120,tiltDownDeg:15,alignFirst:!0}):[];let r=0;const m=12;ct.value={onAnimateStart:()=>{r=0},onAnimateStop:()=>{E.resetToCamera(),Tt()}},U.addInput(R,"render_scale",{label:"Render scale",min:.25,max:z,step:.25}).on("change",X=>{const at=Math.max(.25,X.value),it=Math.max(1,Math.ceil(at*o.clientWidth)),gt=Math.max(1,Math.ceil(at*o.clientHeight));(o.width!==it||o.height!==gt)&&(o.width=it,o.height=gt,L())}),U.addInput(R,"gaussian_scaling",{label:"Surfel scale",min:0,max:1}).on("change",X=>Fr(X.value,f,pt.render_settings_buffer)),U.addInput(R,"sh_bias",{label:"SH bias",min:0,max:2,step:.01}).on("change",X=>qr(X.value,f,pt.render_settings_buffer)),U.addInput(R,"bg",{label:"Background",color:{type:"float",alpha:!0}}).on("change",X=>{pt.bgColor=[X.value.r,X.value.g,X.value.b,X.value.a]});const u=et?et.meta.format===3?"Atlas (ASTC)":et.meta.format===5?"Atlas (RVQ)":"Atlas (BC7)":"Atlas";U.addInput(R,"atlas_enabled",{label:u}).on("change",X=>pt.setAtlasEnabled(X.value)),U.addInput(R,"bfc",{label:"Backface"}).on("change",X=>Zl({bfc:X.value},f,pt.render_settings_buffer)),U.addButton({title:"🎯 Reset camera"}).on("click",()=>{if(Y.length>0)x.set_preset(Y[0]);else{const X=Q.bbox.max[0]-Q.bbox.min[0],at=Q.bbox.max[1]-Q.bbox.min[1],it=Q.bbox.max[2]-Q.bbox.min[2],Pt=.5*Math.sqrt(X*X+at*at+it*it)*.5;I.set(_t[0]-Pt,_t[1]-Pt,_t[2]-Pt,x.position);const Et=I.create(Pt,Pt,Pt);I.normalize(Et,Et);const mt=I.create();I.cross(I.create(0,1,0),Et,mt),I.normalize(mt,mt);const vt=I.create();I.cross(Et,mt,vt);const Gt=he.create(mt[0],vt[0],Et[0],mt[1],vt[1],Et[1],mt[2],vt[2],Et[2]);jt.fromMat3(Gt,x.rotation),x.update_buffer()}E.resetToCamera(),Tt()});const p={result:"— click Benchmark —"};U.addMonitor(p,"result",{label:"Bench",interval:500,multiline:!0,lineCount:4});const a={bicycle:{w:1237,h:822,fovY:2*Math.atan(3286/(2*4627.3))},flowers:{w:1256,h:828,fovY:2*Math.atan(3312/(2*4285.5))},garden:{w:1297,h:840,fovY:2*Math.atan(3361/(2*3852.4))},stump:{w:1245,h:825,fovY:2*Math.atan(3300/(2*4528.1))},treehill:{w:1267,h:832,fovY:2*Math.atan(3326/(2*4205.6))},bonsai:{w:1559,h:1039,fovY:2*Math.atan(2078/(2*3222.7))},counter:{w:1558,h:1038,fovY:2*Math.atan(2076/(2*3192.7))},kitchen:{w:1558,h:1039,fovY:2*Math.atan(2078/(2*3240.8))},room:{w:1557,h:1038,fovY:2*Math.atan(2075/(2*3174))}};function _(){const at=((new URLSearchParams(window.location.search).get("bundle")??"").split("/").pop()??"").toLowerCase();for(const it of Object.keys(a))if(at.startsWith(it))return it;return null}const w=document.createElement("div");w.id="bench-overlay",w.style.cssText=["position:fixed","top:50%","left:50%","transform:translate(-50%,-50%)","background:rgba(0,0,0,0.9)","color:#fff","padding:24px 32px","border-radius:8px","font-family:monospace","font-size:14px","min-width:340px","text-align:left","box-shadow:0 4px 24px rgba(0,0,0,0.6)","display:none","z-index:9999","pointer-events:none"].join(";"),document.body.appendChild(w);function v(X,at,it){const gt=Math.floor(at/Math.max(1,it)*100),Pt=32,Et=Math.floor(at/Math.max(1,it)*Pt),mt="█".repeat(Et)+"░".repeat(Pt-Et);w.innerHTML=`<div style="margin-bottom:10px;font-weight:bold">📊 ${X}</div><div>[${mt}] ${gt}%</div><div style="margin-top:6px;font-size:11px;opacity:0.7">${at} / ${it} frames · offscreen · pipelined · no vsync</div>`,w.style.display="block"}function T(){w.style.display="none"}async function N(X=10,at=200){if(wt)return;if(Y.length===0){p.result="no cameras to benchmark";return}wt=!0;const it=$,gt=R.animate,Pt=new Float32Array(x.position),Et=new Float32Array(x.rotation);$=!1,R.animate=!1,U.refresh(),ht=null,E.enabled=!1;const mt=_(),vt=mt?a[mt]:null,Gt=(vt==null?void 0:vt.w)??o.width,Ut=(vt==null?void 0:vt.h)??o.height,qt=(vt==null?void 0:vt.fovY)??x.getFov(),Kt=mt?`${mt} · ${Gt>=4e3/4+500?"images_4":"images_2"}`:"custom",Xt=o.width,Ht=o.height,ee=x.getFov();o.width=Gt,o.height=Ut,x.setFov(qt),vn(Gt,Ut,f,pt.render_settings_buffer);const Qt=f.createTexture({size:[Gt,Ut,1],format:G,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.STORAGE_BINDING}),be=Qt.createView(),fe=()=>{const Jt=f.createCommandEncoder();pt.frame(Jt,be,!1),f.queue.submit([Jt.finish()])},_e=()=>new Promise(Jt=>setTimeout(Jt,0)),me=20,ve=async(Jt,ue)=>{let ge=0,oe=0;for(v(ue,0,Jt),await _e();oe<Jt;){const we=Math.min(me,Jt-oe),xe=performance.now();for(let je=0;je<we;je++)x.set_preset(Y[(oe+je)%Y.length]),fe();await f.queue.onSubmittedWorkDone();const Yt=performance.now();ge+=Yt-xe,oe+=we,v(ue,oe,Jt),await _e()}return ge};try{await ve(X,"Warming up");const ue=await ve(at,"Benchmarking")/at,ge=1e3/ue,oe=Q.num_points??Q.surfel_data.length/8,we=(qt*180/Math.PI).toFixed(1),xe=`${ge.toFixed(1)} FPS  (${ue.toFixed(2)} ms/frame)
${Gt}×${Ut} · fovY ${we}° · ${Kt}
${oe.toLocaleString()} surfels · ${X}w+${at}b · pipelined`;p.result=xe,console.log("[bench]",xe.replace(/\n/g,"  |  "))}catch(Jt){console.error("[bench] failed:",Jt),p.result=`bench failed: ${Jt}`}finally{T(),Qt.destroy(),o.width=Xt,o.height=Ht,x.setFov(ee),vn(Xt,Ht,f,pt.render_settings_buffer),x.position.set(Pt),x.rotation.set(Et),x.update_buffer(),E.enabled=!it,$=it,R.animate=gt,U.refresh(),wt=!1}}U.addButton({title:"📊 Benchmark"}).on("click",()=>N()),document.addEventListener("keydown",X=>{const at=X.key;if(at>="0"&&at<="9"&&Y.length>0){const it=parseInt(at);it<Y.length&&y(it)}else at==="ArrowLeft"||at==="PageUp"?(y(i-1),X.preventDefault()):at==="ArrowRight"||at==="PageDown"?(y(i+1),X.preventDefault()):(at==="d"||at==="D")&&pt.debugReadSortedIndices(30).catch(it=>console.error("[DEBUG] readback failed:",it))});function W(X,at){const it=o.getBoundingClientRect(),gt=window.devicePixelRatio||1;return[(X-it.left)*gt,(at-it.top)*gt]}o.addEventListener("dblclick",X=>{const[at,it]=W(X.clientX,X.clientY);Rt(at,it)});let ut=0,ot=0,rt=0;o.addEventListener("pointerdown",X=>{if(X.pointerType!=="touch")return;const at=performance.now(),it=at-ut,gt=X.clientX-ot,Pt=X.clientY-rt;if(it>0&&it<300&&gt*gt+Pt*Pt<40*40){const[Et,mt]=W(X.clientX,X.clientY);Rt(Et,mt),ut=0}else ut=at,ot=X.clientX,rt=X.clientY});function ft(){return M}let yt=performance.now(),dt=60,bt=Promise.resolve(),Ct=0;async function At(){var Et;const X=performance.now(),at=Math.min((X-yt)/1e3,.1);if(yt=X,at>0){const mt=((Et=pt.lastStageBreakdownMs)==null?void 0:Et.total)??0,vt=mt>.5?1e3/mt:1/at;dt=dt*.9+vt*.1,It.stats=`${st} surfels · ${Math.round(dt)} fps`}if(wt||kt){requestAnimationFrame(At);return}if(ft()&&(ht||$)&&(ht=null,E.resetToCamera(),Tt(),$&&($=!1,R.animate=!1,U.refresh())),$&&R.animateMode==="circle"&&S){r+=at/m,r>=1&&(r-=1);const mt=ii(S,r);x.set_preset(mt),E.update(at);const vt=f.createCommandEncoder();pt.frame(vt,l.getCurrentTexture().createView()),f.queue.submit([vt.finish()]),Ct++,Ct===2&&gn(),requestAnimationFrame(At);return}if(ht){ht.t+=at/ht.duration;const mt=Math.min(1,ht.t),vt=mt*mt*(3-2*mt);I.lerp(ht.fromPos,ht.toPos,vt,x.position),se.slerp(ht.fromQuat,ht.toQuat,vt,j),he.fromQuat(j,g),jt.fromMat3(g,x.rotation),x.update_buffer(),ht.t>=1&&(x.set_preset(ht.target),ht=null,$?Y.length>0&&(Wt=xt):(E.resetToCamera(),Tt()))}else if($&&!ft()){const mt=R.animateMode==="circle"&&k.length>0,vt=mt?k:Y;if(vt.length!==0){if(Wt-=at,Wt<=0){const Ut=((mt?B:i)+1)%vt.length;mt?B=Ut:i=Ut;const qt=mt?Ot/8:Ot;b(vt[Ut],qt),mt||(h.view=`${i+1} / ${Y.length}: ${Y[i].img_name??i}`)}}}E.update(at),J(),await bt;const it=f.createCommandEncoder(),gt=l.getCurrentTexture().createView(),Pt=window.__halloumiProgress??(mt=>{});Ct<3&&Pt("flat-frame#"+Ct+" encoding…"),pt.frame(it,gt),Ct<3&&Pt("flat-frame#"+Ct+" submitting…"),f.queue.submit([it.finish()]),Ct<3&&Pt("flat-frame#"+Ct+" submit OK"),bt=f.queue.onSubmittedWorkDone().then(()=>{Ct<=3&&Pt("flat-frame#"+(Ct-1)+" GPU work done")}),Ct++,Ct===2&&gn(),requestAnimationFrame(At)}requestAnimationFrame(At)}}(function(){let l="dev";for(const C of Array.from(document.querySelectorAll('script[type="module"]'))){const E=C.src.match(/\/assets\/index-([0-9a-z]+)\.js$/i);if(E){l=E[1];break}}const f=document.createElement("div");f.textContent="v "+l,f.title="viewer build hash (Vite content hash of index-*.js)",Object.assign(f.style,{position:"fixed",right:"6px",bottom:"6px",font:"10px ui-monospace, SFMono-Regular, Menlo, monospace",color:"rgba(255,255,255,0.55)",background:"rgba(0,0,0,0.35)",padding:"2px 6px",borderRadius:"4px",pointerEvents:"none",zIndex:"9999",userSelect:"all"}),document.body.appendChild(f)})();function Oe(o){let l=document.getElementById("halloumi-xr-error");l||(l=document.createElement("div"),l.id="halloumi-xr-error",l.style.cssText="position:fixed;top:0;left:0;right:0;padding:1em;background:#3a0000;color:#ffb0b0;font-family:system-ui,monospace;font-size:0.85rem;z-index:9999;white-space:pre-wrap;overflow-x:auto;max-height:60vh;border-bottom:2px solid #7a0000",document.body.appendChild(l)),l.textContent+=(l.textContent?`
`:"")+o,console.error(o)}function qe(o){console.log(o)}window.__halloumiProgress=qe;(async()=>{var o;try{if(qe("boot: main.ts entered"),navigator.gpu===void 0){Oe("WebGPU is not supported in this browser. (navigator.gpu is undefined)");return}qe("requesting adapter…");const l=await navigator.gpu.requestAdapter({powerPreference:"high-performance"});if(l===null){Oe("WebGPU adapter unavailable. requestAdapter returned null.");return}qe("adapter OK; vendor="+(((o=l.info)==null?void 0:o.vendor)??"?"));const f=[];l.features.has("timestamp-query")&&f.push("timestamp-query"),l.features.has("texture-compression-bc")&&f.push("texture-compression-bc"),l.features.has("texture-compression-astc")&&f.push("texture-compression-astc"),console.log("[adapter]",l.info??"(unknown)"),console.log("[adapter] features:",Array.from(l.features)),console.log("[adapter] BC7:",l.features.has("texture-compression-bc")),console.log("[adapter] ASTC:",l.features.has("texture-compression-astc")),console.log("[adapter] limits:",{maxStorageBuffersPerShaderStage:l.limits.maxStorageBuffersPerShaderStage,maxComputeWorkgroupStorageSize:l.limits.maxComputeWorkgroupStorageSize,maxBufferSize:l.limits.maxBufferSize,maxStorageBufferBindingSize:l.limits.maxStorageBufferBindingSize,maxTextureDimension2D:l.limits.maxTextureDimension2D});const C=10,x=Math.min(C,l.limits.maxStorageBuffersPerShaderStage);x<C&&console.warn(`[adapter] adapter exposes only ${x} storage buffers per stage; viewer wants ${C}. Cull stage may fail to compile.`),qe("requesting device (features="+f.join(",")+")…");let E;try{E=await l.requestDevice({requiredFeatures:f,requiredLimits:{maxStorageBuffersPerShaderStage:x,maxComputeWorkgroupStorageSize:l.limits.maxComputeWorkgroupStorageSize,maxBufferSize:l.limits.maxBufferSize,maxStorageBufferBindingSize:l.limits.maxStorageBufferBindingSize}}),qe("device OK")}catch(A){Oe(`WebGPU device creation FAILED.
error: ${A.message}
...
${L.message}`)}),E.lost.then(A=>{Oe(`WebGPU device LOST: ${A.reason} — ${A.message}`)});const M=document.querySelector("#webgpu-canvas");Dl(M!==null);const G=M.getContext("webgpu");if(G===null){Oe('canvas.getContext("webgpu") returned null.');return}qe("canvas ctx OK; entering splat-app init…"),Ic(M,G,E,f)}catch(l){Oe(`Bootstrap failed: ${l.message}
${l.stack??""}`)}})();
