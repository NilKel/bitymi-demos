...
${wt.toFixed(1)} MB BC7`);try{const H=Il(pt.atlasBuffer);Vt=Gl(h,H,!0)}catch(H){console.warn("[atlas] failed to parse/upload atlas:",H)}}}catch(pt){throw gn(),pt}}else _t=await zl(X,h);const Tt=et?await cc(et):[];Tt.length>0&&x.set_preset(Tt[0]),K(_t,Tt,Vt)});function K(X,Y=[],et=null){const _t=[(X.bbox.min[0]+X.bbox.max[0])/2,(X.bbox.min[1]+X.bbox.max[1])/2,(X.bbox.min[2]+X.bbox.max[2])/2];E.setBbox(X.bbox.min,X.bbox.max);const Vt=.5*Math.sqrt((X.bbox.max[0]-X.bbox.min[0])**2+(X.bbox.max[1]-X.bbox.min[1])**2+(X.bbox.max[2]-X.bbox.min[2])**2);function Rt(Q,lt){const ot=Wr(Q,lt,o,x,X.surfel_data,Vt);ot&&(E.setOrbitPivot(ot),console.log(`[pick] orbit pivot → (${ot[0].toFixed(3)}, ${ot[1].toFixed(3)}, ${ot[2].toFixed(3)})`))}function Tt(){const Q=o.width*.5,lt=o.height*.5,ot=Wr(Q,lt,o,x,X.surfel_data,Vt);if(!ot)return;const gt=x.rotation,Pt=gt[2],Et=gt[6],mt=gt[10],vt=ot[0]-x.position[0],Gt=ot[1]-x.position[1],Ut=ot[2]-x.position[2],qt=vt*Pt+Gt*Et+Ut*mt;qt>0&&E.setOrbitDepth(qt)}if(Y.length===0){const Q=X.bbox.max[0]-X.bbox.min[0],lt=X.bbox.max[1]-X.bbox.min[1],ot=X.bbox.max[2]-X.bbox.min[2],Pt=.5*Math.sqrt(Q*Q+lt*lt+ot*ot)*.5;I.set(_t[0]-Pt,_t[1]-Pt,_t[2]-Pt,x.position);const Et=I.create(Pt,Pt,Pt);I.normalize(Et,Et);const mt=I.create(0,1,0),vt=I.create();I.cross(mt,Et,vt),I.normalize(vt,vt);const Gt=I.create();I.cross(Et,vt,Gt);const Ut=he.create(vt[0],Gt[0],Et[0],vt[1],Gt[1],Et[1],vt[2],Gt[2],Et[2]);jt.fromMat3(Ut,x.rotation),x.update_buffer()}E.setCenter(I.create(X.centroid[0],X.centroid[1],X.centroid[2]));const pt=new oc(X,h,G,x.uniform_buffer,C,et);A=pt;let kt=!1;(async()=>{if(!await Pc())return;const Q=document.getElementById("enter-vr-button");Q&&(Q.style.display="",Q.addEventListener("click",async()=>{if(!kt){kt=!0,Q.disabled=!0,Q.textContent="🥽 In VR…";try{await Sc({device:h,renderer:pt,camera:x,onExit:()=>{kt=!1,Q.disabled=!1,Q.textContent="🥽 Enter VR",vn(o.width,o.height,h,pt.render_settings_buffer),x.on_update_canvas()}})}catch(lt){console.error("[XR] session failed:",lt),alert(`XR session failed: ${lt.message}`),kt=!1,Q.disabled=!1,Q.textContent="🥽 Enter VR"}}}))})(),window.__xrActive=()=>kt,vn(o.width,o.height,h,pt.render_settings_buffer),R.atlas_enabled=et!==null;let wt=!1;const H=(()=>{if(et!==null)return`${et.meta.format===2?"BC7":et.meta.format===3?"ASTC 4×4":et.meta.format===5?"RVQ-paired (typeA)":et.meta.format===6?"RVQ-paired + BC7 codebook (typeB)":et.meta.format===7?"BC7 codebook gather (typeD)":`format=${et.meta.format}`} ${et.meta.width}×${et.meta.height}, ${et.meta.n_layers} layers`;const Q=h.features.has("texture-compression-bc"),lt=h.features.has("texture-compression-astc");return`no atlas in bundle (GPU supports: ${(Q?["BC7"]:[]).concat(lt?["ASTC"]:[]).join("+")||"none"})`})();console.log("[atlas]",H),qr(X.sh_bias,h,pt.render_settings_buffer),Fr(R.gaussian_scaling,h,pt.render_settings_buffer),R.sh_bias=X.sh_bias;const st=X.num_points.toLocaleString(),It={stats:`${st} surfels · -- fps`};U.addMonitor(It,"stats",{label:"Stats",interval:200}),U.addMonitor({atlas:H},"atlas",{label:"Atlas"});const Mt={stages:"— ms · awaiting timestamp data"};U.addMonitor(Mt,"stages",{label:"Stages",interval:500});let zt=null;if(pt.timeQueryEnabled){zt=document.createElement("canvas"),zt.width=280,zt.height=24,zt.style.cssText="position:fixed;right:8px;bottom:8px;width:280px;height:24px;background:#0008;border:1px solid #2a2a2a;border-radius:4px;font-family:ui-monospace,Menlo,monospace;pointer-events:none;z-index:1000;",document.body.appendChild(zt);const Q=ot=>{const gt=zt.getContext("2d");if(!gt)return;const Pt=zt.width,Et=zt.height;if(gt.clearRect(0,0,Pt,Et),!ot||ot.total<=0){gt.fillStyle="#888",gt.font="11px ui-monospace,Menlo,monospace",gt.fillText("awaiting GPU timestamps…",8,16);return}const mt=[{label:"cull",ms:ot.cull,color:"#3aa3ff"},{label:"pre",ms:ot.preprocess,color:"#3ad27a"},{label:"sort",ms:ot.sort,color:"#ffa53a"},{label:"render",ms:ot.render,color:"#ff5566"}],vt=Math.max(.001,mt.reduce((Ut,qt)=>Ut+qt.ms,0));let Gt=0;gt.font="10px ui-monospace,Menlo,monospace",gt.textBaseline="middle";for(const Ut of mt){const qt=Ut.ms/vt*Pt;gt.fillStyle=Ut.color,gt.fillRect(Gt,0,qt,Et),qt>=38&&(gt.fillStyle="#000c",gt.fillText(`${Ut.label} ${Ut.ms.toFixed(1)}`,Gt+4,Et/2)),Gt+=qt}};setInterval(()=>{wt||pt.readPerfMetrics({silent:!0}).then(()=>{const ot=pt.lastStageBreakdownMs;Q(ot),ot&&(Mt.stages=`${ot.total.toFixed(1)} ms · cull ${ot.cull.toFixed(1)} / pre ${ot.preprocess.toFixed(1)} / sort ${ot.sort.toFixed(1)} / render ${ot.render.toFixed(1)}`)}).catch(ot=>console.warn("[perf] readPerfMetrics failed:",ot))},500)}const Dt=.4,Ot=3,xt=.3;let ht=null,Wt=0,B=0;const j=se.create(),g=he.create();let i=Y.length>0?0:-1;const f={view:Y.length>0?`${i+1} / ${Y.length}: ${Y[i].img_name??i}`:"— no presets —"};U.addMonitor(f,"view",{label:"View",interval:100});function d(Q){const lt=he.create(Q[0],Q[1],Q[2],Q[4],Q[5],Q[6],Q[8],Q[9],Q[10]);return se.fromMat(lt)}function b(Q,lt){ht={fromPos:I.clone(x.position),toPos:I.clone(Q.position),fromQuat:se.normalize(d(x.rotation)),toQuat:se.normalize(d(Q.rotation)),target:Q,t:0,duration:Math.max(.01,lt)}}const y=(Q,lt=!0)=>{if(Y.length===0)return;i=(Q%Y.length+Y.length)%Y.length;const ot=Y[i];lt?b(ot,Dt):(x.set_preset(ot),E.resetToCamera(),Tt()),f.view=`${i+1} / ${Y.length}: ${Y[i].img_name??i}`};Y.length>0&&(U.addButton({title:"◀ Prev view"}).on("click",()=>y(i-1)),U.addButton({title:"Next view ▶"}).on("click",()=>y(i+1)));const S=Y.length>0?ii(Y,{tiltDownDeg:15,alignFirst:!0}):null,k=S?yc(Y,{numViews:120,tiltDownDeg:15,alignFirst:!0}):[];let r=0;const m=12;ct.value={onAnimateStart:()=>{r=0},onAnimateStop:()=>{E.resetToCamera(),Tt()}},U.addInput(R,"render_scale",{label:"Render scale",min:.25,max:z,step:.25}).on("change",Q=>{const lt=Math.max(.25,Q.value),ot=Math.max(1,Math.ceil(lt*o.clientWidth)),gt=Math.max(1,Math.ceil(lt*o.clientHeight));(o.width!==ot||o.height!==gt)&&(o.width=ot,o.height=gt,L())}),U.addInput(R,"gaussian_scaling",{label:"Surfel scale",min:0,max:1}).on("change",Q=>Fr(Q.value,h,pt.render_settings_buffer)),U.addInput(R,"sh_bias",{label:"SH bias",min:0,max:2,step:.01}).on("change",Q=>qr(Q.value,h,pt.render_settings_buffer)),U.addInput(R,"bg",{label:"Background",color:{type:"float",alpha:!0}}).on("change",Q=>{pt.bgColor=[Q.value.r,Q.value.g,Q.value.b,Q.value.a]});const u=et?et.meta.format===3?"Atlas (ASTC)":et.meta.format===5?"Atlas (RVQ)":"Atlas (BC7)":"Atlas";U.addInput(R,"atlas_enabled",{label:u}).on("change",Q=>pt.setAtlasEnabled(Q.value)),U.addInput(R,"bfc",{label:"Backface"}).on("change",Q=>Xl({bfc:Q.value},h,pt.render_settings_buffer)),U.addButton({title:"🎯 Reset camera"}).on("click",()=>{if(Y.length>0)x.set_preset(Y[0]);else{const Q=X.bbox.max[0]-X.bbox.min[0],lt=X.bbox.max[1]-X.bbox.min[1],ot=X.bbox.max[2]-X.bbox.min[2],Pt=.5*Math.sqrt(Q*Q+lt*lt+ot*ot)*.5;I.set(_t[0]-Pt,_t[1]-Pt,_t[2]-Pt,x.position);const Et=I.create(Pt,Pt,Pt);I.normalize(Et,Et);const mt=I.create();I.cross(I.create(0,1,0),Et,mt),I.normalize(mt,mt);const vt=I.create();I.cross(Et,mt,vt);const Gt=he.create(mt[0],vt[0],Et[0],mt[1],vt[1],Et[1],mt[2],vt[2],Et[2]);jt.fromMat3(Gt,x.rotation),x.update_buffer()}E.resetToCamera(),Tt()});const p={result:"— click Benchmark —"};U.addMonitor(p,"result",{label:"Bench",interval:500,multiline:!0,lineCount:4});const a={bicycle:{w:1237,h:822,fovY:2*Math.atan(3286/(2*4627.3))},flowers:{w:1256,h:828,fovY:2*Math.atan(3312/(2*4285.5))},garden:{w:1297,h:840,fovY:2*Math.atan(3361/(2*3852.4))},stump:{w:1245,h:825,fovY:2*Math.atan(3300/(2*4528.1))},treehill:{w:1267,h:832,fovY:2*Math.atan(3326/(2*4205.6))},bonsai:{w:1559,h:1039,fovY:2*Math.atan(2078/(2*3222.7))},counter:{w:1558,h:1038,fovY:2*Math.atan(2076/(2*3192.7))},kitchen:{w:1558,h:1039,fovY:2*Math.atan(2078/(2*3240.8))},room:{w:1557,h:1038,fovY:2*Math.atan(2075/(2*3174))}};function _(){const lt=((new URLSearchParams(window.location.search).get("bundle")??"").split("/").pop()??"").toLowerCase();for(const ot of Object.keys(a))if(lt.startsWith(ot))return ot;return null}const w=document.createElement("div");w.id="bench-overlay",w.style.cssText=["position:fixed","top:50%","left:50%","transform:translate(-50%,-50%)","background:rgba(0,0,0,0.9)","color:#fff","padding:24px 32px","border-radius:8px","font-family:monospace","font-size:14px","min-width:340px","text-align:left","box-shadow:0 4px 24px rgba(0,0,0,0.6)","display:none","z-index:9999","pointer-events:none"].join(";"),document.body.appendChild(w);function v(Q,lt,ot){const gt=Math.floor(lt/Math.max(1,ot)*100),Pt=32,Et=Math.floor(lt/Math.max(1,ot)*Pt),mt="█".repeat(Et)+"░".repeat(Pt-Et);w.innerHTML=`<div style="margin-bottom:10px;font-weight:bold">📊 ${Q}</div><div>[${mt}] ${gt}%</div><div style="margin-top:6px;font-size:11px;opacity:0.7">${lt} / ${ot} frames · offscreen · pipelined · no vsync</div>`,w.style.display="block"}function T(){w.style.display="none"}async function N(Q=10,lt=200){if(wt)return;if(Y.length===0){p.result="no cameras to benchmark";return}wt=!0;const ot=$,gt=R.animate,Pt=new Float32Array(x.position),Et=new Float32Array(x.rotation);$=!1,R.animate=!1,U.refresh(),ht=null,E.enabled=!1;const mt=_(),vt=mt?a[mt]:null,Gt=(vt==null?void 0:vt.w)??o.width,Ut=(vt==null?void 0:vt.h)??o.height,qt=(vt==null?void 0:vt.fovY)??x.getFov(),Kt=mt?`${mt} · ${Gt>=4e3/4+500?"images_4":"images_2"}`:"custom",Xt=o.width,Ht=o.height,ee=x.getFov();o.width=Gt,o.height=Ut,x.setFov(qt),vn(Gt,Ut,h,pt.render_settings_buffer);const Qt=h.createTexture({size:[Gt,Ut,1],format:G,usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.STORAGE_BINDING}),be=Qt.createView(),fe=()=>{const Jt=h.createCommandEncoder();pt.frame(Jt,be,!1),h.queue.submit([Jt.finish()])},_e=()=>new Promise(Jt=>setTimeout(Jt,0)),me=20,ve=async(Jt,ue)=>{let ge=0,oe=0;for(v(ue,0,Jt),await _e();oe<Jt;){const we=Math.min(me,Jt-oe),xe=performance.now();for(let je=0;je<we;je++)x.set_preset(Y[(oe+je)%Y.length]),fe();await h.queue.onSubmittedWorkDone();const Yt=performance.now();ge+=Yt-xe,oe+=we,v(ue,oe,Jt),await _e()}return ge};try{await ve(Q,"Warming up");const ue=await ve(lt,"Benchmarking")/lt,ge=1e3/ue,oe=X.num_points??X.surfel_data.length/8,we=(qt*180/Math.PI).toFixed(1),xe=`${ge.toFixed(1)} FPS  (${ue.toFixed(2)} ms/frame)
${Gt}×${Ut} · fovY ${we}° · ${Kt}
${oe.toLocaleString()} surfels · ${Q}w+${lt}b · pipelined`;p.result=xe,console.log("[bench]",xe.replace(/\n/g,"  |  "))}catch(Jt){console.error("[bench] failed:",Jt),p.result=`bench failed: ${Jt}`}finally{T(),Qt.destroy(),o.width=Xt,o.height=Ht,x.setFov(ee),vn(Xt,Ht,h,pt.render_settings_buffer),x.position.set(Pt),x.rotation.set(Et),x.update_buffer(),E.enabled=!ot,$=ot,R.animate=gt,U.refresh(),wt=!1}}U.addButton({title:"📊 Benchmark"}).on("click",()=>N()),document.addEventListener("keydown",Q=>{const lt=Q.key;if(lt>="0"&&lt<="9"&&Y.length>0){const ot=parseInt(lt);ot<Y.length&&y(ot)}else lt==="ArrowLeft"||lt==="PageUp"?(y(i-1),Q.preventDefault()):lt==="ArrowRight"||lt==="PageDown"?(y(i+1),Q.preventDefault()):(lt==="d"||lt==="D")&&pt.debugReadSortedIndices(30).catch(ot=>console.error("[DEBUG] readback failed:",ot))});function W(Q,lt){const ot=o.getBoundingClientRect(),gt=window.devicePixelRatio||1;return[(Q-ot.left)*gt,(lt-ot.top)*gt]}o.addEventListener("dblclick",Q=>{const[lt,ot]=W(Q.clientX,Q.clientY);Rt(lt,ot)});let ut=0,it=0,rt=0;o.addEventListener("pointerdown",Q=>{if(Q.pointerType!=="touch")return;const lt=performance.now(),ot=lt-ut,gt=Q.clientX-it,Pt=Q.clientY-rt;if(ot>0&&ot<300&&gt*gt+Pt*Pt<40*40){const[Et,mt]=W(Q.clientX,Q.clientY);Rt(Et,mt),ut=0}else ut=lt,it=Q.clientX,rt=Q.clientY});function ft(){return M}let yt=performance.now(),dt=60,bt=Promise.resolve(),Ct=0;async function At(){var Et;const Q=performance.now(),lt=Math.min((Q-yt)/1e3,.1);if(yt=Q,lt>0){const mt=((Et=pt.lastStageBreakdownMs)==null?void 0:Et.total)??0,vt=mt>.5?1e3/mt:1/lt;dt=dt*.9+vt*.1,It.stats=`${st} surfels · ${Math.round(dt)} fps`}if(wt||kt){requestAnimationFrame(At);return}if(ft()&&(ht||$)&&(ht=null,E.resetToCamera(),Tt(),$&&($=!1,R.animate=!1,U.refresh())),$&&R.animateMode==="circle"&&S){r+=lt/m,r>=1&&(r-=1);const mt=oi(S,r);x.set_preset(mt),E.update(lt);const vt=h.createCommandEncoder();pt.frame(vt,l.getCurrentTexture().createView()),h.queue.submit([vt.finish()]),Ct++,Ct===2&&gn(),requestAnimationFrame(At);return}if(ht){ht.t+=lt/ht.duration;const mt=Math.min(1,ht.t),vt=mt*mt*(3-2*mt);I.lerp(ht.fromPos,ht.toPos,vt,x.position),se.slerp(ht.fromQuat,ht.toQuat,vt,j),he.fromQuat(j,g),jt.fromMat3(g,x.rotation),x.update_buffer(),ht.t>=1&&(x.set_preset(ht.target),ht=null,$?Y.length>0&&(Wt=xt):(E.resetToCamera(),Tt()))}else if($&&!ft()){const mt=R.animateMode==="circle"&&k.length>0,vt=mt?k:Y;if(vt.length!==0){if(Wt-=lt,Wt<=0){const Ut=((mt?B:i)+1)%vt.length;mt?B=Ut:i=Ut;const qt=mt?Ot/8:Ot;b(vt[Ut],qt),mt||(f.view=`${i+1} / ${Y.length}: ${Y[i].img_name??i}`)}}}E.update(lt),J(),await bt;const ot=h.createCommandEncoder(),gt=l.getCurrentTexture().createView(),Pt=window.__halloumiProgress??(mt=>{});Ct<3&&Pt("flat-frame#"+Ct+" encoding…"),pt.frame(ot,gt),Ct<3&&Pt("flat-frame#"+Ct+" submitting…"),h.queue.submit([ot.finish()]),Ct<3&&Pt("flat-frame#"+Ct+" submit OK"),bt=h.queue.onSubmittedWorkDone().then(()=>{Ct<=3&&Pt("flat-frame#"+(Ct-1)+" GPU work done")}),Ct++,Ct===2&&gn(),requestAnimationFrame(At)}requestAnimationFrame(At)}}(function(){let l="dev";for(const C of Array.from(document.querySelectorAll('script[type="module"]'))){const E=C.src.match(/\/assets\/index-([0-9a-z]+)\.js$/i);if(E){l=E[1];break}}const h=document.createElement("div");h.textContent="v "+l,h.title="viewer build hash (Vite content hash of index-*.js)",Object.assign(h.style,{position:"fixed",right:"6px",bottom:"6px",font:"10px ui-monospace, SFMono-Regular, Menlo, monospace",color:"rgba(255,255,255,0.55)",background:"rgba(0,0,0,0.35)",padding:"2px 6px",borderRadius:"4px",pointerEvents:"none",zIndex:"9999",userSelect:"all"}),document.body.appendChild(h)})();function Oe(o){let l=document.getElementById("halloumi-xr-error");l||(l=document.createElement("div"),l.id="halloumi-xr-error",l.style.cssText="position:fixed;top:0;left:0;right:0;padding:1em;background:#3a0000;color:#ffb0b0;font-family:system-ui,monospace;font-size:0.85rem;z-index:9999;white-space:pre-wrap;overflow-x:auto;max-height:60vh;border-bottom:2px solid #7a0000",document.body.appendChild(l)),l.textContent+=(l.textContent?`
...
${L.message}`)}),E.lost.then(A=>{Oe(`WebGPU device LOST: ${A.reason} — ${A.message}`)});const M=document.querySelector("#webgpu-canvas");Tl(M!==null);const G=M.getContext("webgpu");if(G===null){Oe('canvas.getContext("webgpu") returned null.');return}qe("canvas ctx OK; entering splat-app init…"),Lc(M,G,E,h)}catch(l){Oe(`Bootstrap failed: ${l.message}
${l.stack??""}`)}})();
